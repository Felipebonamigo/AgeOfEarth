...
- Mapas oficiais: `npx tsx scripts/maps/estreito.ts` / `egeu.ts` regeram `src/core/data/maps/<id>.map.json` com as ops do editor (simetria e justiça conferidas; `tests/data.test.ts` exige que o arquivo seja exatamente o gerado); `npx tsx scripts/maps/fairness.ts <mapa> 35 1,2,3,4 zeus` mede vitórias/idades por início
