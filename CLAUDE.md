...
- `node scripts/playtest.mjs` / `playtest-campaign.mjs` / `playtest-mp.mjs` / `playtest-horde-replay.mjs` / `actionshot.mjs` — Chromium headless (Playwright); exigem `npm run preview` e, para o multiplayer, `npm run relay`
...
- **Estado atual**: Fase 0 concluída (fatia vertical: skirmish, campanha com 3 missões, Horda, multiplayer lockstep, replays, Electron). Em andamento: Fase 1 (jogabilidade sólida). Próximos da lista: guarnição, portões, formações, IA "Muito difícil", balanceamento, i18n PT/EN, opções.
