...
| `npm test` | 81 testes: dados, determinismo, pathfinding, simulação, regressões, cenários, lockstep/reconexão, modos | ~5 s |
...
| `node scripts/playtest-options.mjs`, `playtest-i18n.mjs`, `playtest-modes.mjs` | Opções/atalhos/diagnóstico, inglês, modos de jogo | ~2 min |
| `npm run relay` + `node scripts/playtest-mp.mjs`, `playtest-reconnect.mjs` | Dois navegadores em lockstep, chat, ping, queda e reconexão | ~2 min |
