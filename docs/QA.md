...
| `npm test` | 286 testes: dados (inclui `checkMap` dos mapas embutidos, recursos iguais por início e mapas oficiais reprodutíveis pelos scripts), determinismo, pathfinding, simulação, regressões, cenários, lockstep/reconexão, modos, mapas fixos, editor, qualidade, áudio, controle, shader do terreno | ~15 s |
...
| `npx tsx scripts/maps/estreito.ts` / `scripts/maps/egeu.ts [saída]` | Regeram os mapas oficiais com as ops do editor; recusam gravar com erro de validação, assimetria, recursos desiguais ou vizinhança de início diferente (o teste confere que o arquivo embutido é exatamente o que o script gera) | ~2 s |
| `npx tsx scripts/maps/fairness.ts <mapa> [min] [sementes] [deuses]` | Justiça por início: IA×IA com o mesmo deus (ou em rodízio), vitórias e minutos das idades por início. Referência 25/09/2026 (35 min, Normal): Estreito, 6 sementes Zeus/Poseidon alternados → 3×3 vitórias, Heroica 15,8, Mítica 19,5–20, Titãs 23,5–24,5; Egeu 2v2 por times, 6 sementes 4×Zeus → norte 3, sul 1, 2 empates, Heroica 15,7–17,2, Mítica 20,8–22, Titãs 27–29 | ~3 min |
| `npm run balance 35 1,2,3 -- --map src/core/data/maps/<id>.map.json` | Balanço num mapa fixo (tantas IAs quantos inícios; linha "aos 5 min" acusa IA parada). Referência 25/09/2026: Estreito Clássica 5–6, Heroica 13–19, Mítica 18–25, Titãs 23–27; Egeu (4 IAs, cada um por si) Clássica 5–6, Heroica 14–18, Mítica 18–23, Titãs 23–27; nenhuma parada aos 5 min | ~1 min |
