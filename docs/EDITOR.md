...
- **Justiça de posição (26/09/2026)** — o viés norte do Egeu era do motor/IA, não do mapa (exatamente simétrico): seguia a POSIÇÃO (com os jogadores trocados de início o norte continuava vencendo, 23 × 4 somando 48 partidas) e se invertia no Egeu transposto. Causa: uma soma de regras com orientação absoluta — kit ao sul do CC, `nearestNode`/`findSpawnTile` desempatando pela ordem da varredura (norte/oeste primeiro), distâncias medidas do canto dos nós e das pegadas, espiral de `findBuildSpot` que testava o norte primeiro (templo, mercado e casas sempre ao sul do CC: à frente para quem começa ao norte, atrás para quem começa ao sul), âncora das casas com y absoluto, cidadãos assustados indo para `tc.y + 3`, formação por id — e, tiradas elas, efeitos de ÍNDICE (a IA do jogador 0 pensava 1 s antes, as IAs pensavam sempre na ordem 0→3, a unidade mais antiga golpeava primeiro). Correção: `centerFrame` (`src/core/map/grid.ts`: sinais e eixo dominante do vetor início→centro, sem trigonometria; invariante a espelho nos dois eixos, rotação de 180° e transposição) orienta o kit (fileira que vira coluna em mapas leste×oeste), a âncora das casas e os pontos da Fartura/Isca; `towardFrame` (o lado de quem chega) orienta a espiral dos destinos bloqueados de um grupo (atacar o centro de um edifício), `nearestFreeTile` e o empurrão de unidades sob um edifício novo; desempates pela distância ao centro do mapa (`nearestNode`, `nearestUnclaimedNode`, `findSpawnTile`); centros de tile/pegada em vez de cantos; `findBuildSpot` em ordem de distância âncora→centro da pegada (ordem em cache por pegada, raio e fração do âncora, bit a bit a da ordenação direta); formação invariante a espelho; alvo "mais fraco" desempatado pelo inimigo mais perto (antes, o de menor índice); herói leva a relíquia ao lado do Templo voltado a ele; sem CC inimigo, `enemyDirection` aponta ao centro; todas as IAs começam juntas e a primeira a pensar gira a cada segundo (`firstThinker`); unidades e edifícios em ordem invertida nos ticks ímpares. `SIM_VERSION` = 2 (replays antigos são recusados com mensagem). Testes: `tests/position-fairness.test.ts`.
- Números (IA Normal, 45 min, mesmo deus, critério novo; HEAD → depois): **Egeu** 32 sementes × 2 ordens: por posição 45 × 19 (70 %, **fora**) → 37 × 27 (58 %), por índice 35 × 29 → 35 × 29 (55 %), 59 de 64 decididas; a 35 min (16 sementes), norte 8 × sul 0 na ordem padrão → norte 7 × sul 8 decididas nas duas ordens (19 × 13 com as à frente, 59 %). **Egeu transposto** (times oeste × leste, 16): 5 × 11 (69 %, fora) → 8 × 8. **Estreito** Zeus 32 × 2 ordens: 31 × 33 → 38 × 26 (59 %), índice 32 × 31 → 40 × 24 (63 %), dentro; só na ordem padrão, 16 × 16 → 22 × 8 (72 %): o desequilíbrio acompanha a paridade da semente — a personalidade da IA vem do índice, (semente + 7·i) % 97, e jogadores vizinhos escolhem deuses menores opostos; o lado com a personalidade par vence ~80 % em espelho, antes e agora; com `--mirror-ai` (a mesma personalidade em todos) a ordem padrão dá 20 × 12 (63 %) e as duas ordens, posição 34 × 30 (53 %) e índice 38 × 26 (59 %). Poseidon 16: 6 × 10 → 6 × 10 (63 %). **Mapa gerado** 1v1 (small, semente 42): 16 × 0 antes e depois — o mapa é desigual (ouro a ≤ 16 tiles 3600 × 1800), não é viés de posição do motor. Idades (Egeu, depois): Clássica 5,5–5,7, Heroica 15,9–16,4, Mítica 20,8–21,5. O Egeu pode ser anunciado como justo para 2v2. Pendências: deuses menores por personalidade (desequilíbrio de balanceamento que o espelho por índice expõe), um resto de efeito de índice no 1v1 (jogador 0 com 59 % no Estreito com `--mirror-ai`; o HEAD dava 52 % sem ela), empate exato de território decidido pelo índice, ordem de vizinhos do A* (`DIRS`) e desempates finais (y, x) — nenhum mudou o resultado medido.
