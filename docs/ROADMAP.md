...
| 3.3 | ✅ Mapas fixos (`docs/EDITOR.md`, Etapas 1–2): formato `FixedMapData` com entidades pré-colocadas, kit inicial, times por início, colina e relíquias; forma canônica, hash, validação (erros/avisos traduzidos), migração; `createGame` aplica tudo; hash do estado cobre terreno e nós; biblioteca local (Meus mapas), mapa embutido `estreito`, seletor na Partida rápida e no lobby (validação, limite de oponentes, ordem de inícios por time), importar/exportar/guardar, `scripts/export-map.ts`, `mapcheck.ts`, `smoke -- --map`. Pendente: mapas oficiais desenhados no editor (Etapa 4) | A | 8–9 |
