...
| 3.2 | 🔶 Editor interno (`docs/EDITOR.md` Etapa 3): ✅ operações desfazíveis com inversa exata (`src/editor/ops.ts`, `editor.ts`, 24 testes) e renderização do editor (invalidação por chunk, pincel, fantasmas, inícios, kit, grade, regiões, passabilidade). ✅ interface DOM (aba Editor: novo/rascunho/Meus mapas/importar; painel com paleta, pincel, jogador ativo, inspetor, validação com Ir até/Corrigir; Testar contra IAs e voltar ao editor; salvar/exportar/propriedades; `playtest-editor.mjs`). ✅ Etapa 5 parte 1: cenários declarativos em JSON (`src/core/scenario/schema.ts`, `compile.ts`, `text.ts`; gramática fechada validada, compilação com cache por idioma, missão 1 em JSON com paridade contra a versão TS, 27 testes). ✅ Etapa 5 parte 2: modal Gatilhos no editor (JSON validado, modelos, pegar ponto), Testar com cenário, Cenários personalizados na campanha, cenário no lobby e `scenarioData` validado em todos os clientes, HUD genérico (`def.hud`), `playtest-scenario.mjs`. Pendente: Etapa 4 (retoque dos mapas oficiais no editor, decoração) | A | 7–10 |
| 3.3 | ✅ Mapas fixos (`docs/EDITOR.md`, Etapas 1–2): formato `FixedMapData` com entidades pré-colocadas, kit inicial, times por início, colina e relíquias; forma canônica, hash, validação (erros/avisos traduzidos), migração; `createGame` aplica tudo; hash do estado cobre terreno e nós; biblioteca local (Meus mapas), mapa embutido `estreito`, seletor na Partida rápida e no lobby (validação, limite de oponentes, ordem de inícios por time), importar/exportar/guardar, `scripts/export-map.ts`, `mapcheck.ts`, `smoke/balance -- --map`. Dois mapas embutidos gerados e validados (Estreito 1v1; Egeu 2v2 com times sugeridos: Clássica 5, Heroica 14–20, Mítica 19–23, Titãs 26–28 min). Pendente: retocar os mapas oficiais no editor (Etapa 4) | A | 8–9 |
