...
| 3.2 | 🔶 Editor interno (`docs/EDITOR.md` Etapa 3): ✅ operações desfazíveis com inversa exata (`src/editor/ops.ts`, `editor.ts`, 24 testes) e renderização do editor (invalidação por chunk, pincel, fantasmas, inícios, kit, grade, regiões, passabilidade). Em curso: interface DOM (painel, aba Editor, testar/jogar, playtest). Pendente: Etapa 4 (mapas oficiais, correções assistidas) e Etapa 5 (cenários em JSON) | A | 7–10 |
