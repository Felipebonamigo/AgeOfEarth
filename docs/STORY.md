...
| 9 | `m9_tenaro` *(produzida)* | A Descida ao Tênaro / *The Descent at Taenarum* | **Criaturas míticas, jogando como Hades** (cobra: heróis contra míticas) | resgate + cerco leve | Heroica | gen M lakes, 9909 | 25–30 | — | — (usa G3, G4, G6 `maxAge`, G8, G10 e G11 `powerUsed` ✅) |
...
- **Falas novas** (além das da ficha): a Pítia com a tecla X do Cérbero (75 s); Hades na 1ª onda das fendas; Hades quando o Culto usa o **raio roubado** (`powerUsed`, G11 — a IA do Culto o gasta em Héracles ou Perseu no começo, e a fala diz que ele só cai uma vez); Arges com as três jaulas abertas; Hades ao recuperar o Elmo das Trevas ("foi com ele que Lícaon se escondeu entre os meus sacerdotes"); Hades no prêmio das Sombras. Dicas (`hints`) sobre o Palácio, o triângulo, o raio do Culto, as Sombras, as jaulas e fendas e as relíquias.
- **Passiva:** derrota aos 13m39s / 11m25s / 7m36s (o rascunho media 15/10/8).
...
| m9 | Ondas das fendas até o Palácio cair. **Medido no rascunho:** derrota aos 15, 10 e 8 min (Fácil, Normal, Difícil). **Medido (produção):** derrota aos 13m39s / 11m25s / 7m36s; 12 / 11 / 9 invasões até a derrota, todas com unidades | Míticas e heróis fazem `attackMove` para `jaula1..3` e depois `fenda1..3`. **Produção:** casas antes do teto de população, filas com as míticas do Templo, e um alvo por vez na ordem da caminhada (`fenda1` → `jaula1` → `jaula2` → `jaula3` → `fenda3` → `fenda2`), cada um quando o exército tem ≥ 40 militares (detalhes abaixo) | 22–35 min (§7.2) · janela estrita 17m30s–39m (§4 ±30 %). **Medido:** vitória aos 24m53s / 22m42s / 33m52s (Fácil / Normal / Difícil); `miticas`, `cerbero` e `elmo` cumpridos nas três, segredo `sombras` em nenhuma; 20 / 22 / 50 invasões, todas com unidades; o Culto é eliminado entre os 21 e os 26 min |
