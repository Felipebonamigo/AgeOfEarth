...
| **G10** ✅ | Relíquias em cenário | **Feito.** `map.relics` (cenário em mapa gerado → `config.relics`) e `FixedMapData.relics`: `boolean \| [x, y][]` — lista = posições fixas (tiles, na ordem da lista, até `MAX_FIXED_RELICS` = 32; `[]` = nenhuma), por `placeRelicsAt` (`src/core/sim/relics.ts`, sem `state.rng`; tile que não é terra livre na região dos inícios vai à terra alcançável mais próxima até `RELIC_SNAP_RADIUS` = 6, senão sai). `validateMap`: `relicOut`, `relicBlocked`, `relicUnreachable`, `relicsCount` (erros, PT/EN); `validateScenario`: formato, limites do tamanho, repetidas e `map.relics` junto de `data` (use `data.relics`); lint: gera o mapa pela semente e avisa a posição que não é terra alcançável. `mapHash`/`canonicalize`/redimensionar (`ResizeReport.relics`) e o editor (Propriedades mantém a lista) cobrem a lista | `Value { "stat": "relics", "player" }` = `relicsOf` (guardadas agora nos Templos) | — | m9 (secundário) | **feita** |
