...
| 7 | `m7_aquiles` *(produzida)* | A Cólera de Aquiles / *The Wrath of Achilles* | **Caça ao herói** com o poder Oráculo (cobra: defesa de aldeias) | **caça ao herói** | Heroica | gen M forest, 7707 | 20–30 | ✅ | — (G11 ✅ permitiria trocar o paliativo de Tétis; decisão do dono) |
| 8 | `m8_oceano` *(produzida)* | A Maré de Oceano / *The Tide of Oceanus* | **Chefe-Titã com contagem regressiva** (em três marés), Raio, invocar Prometeu (cobra: muralhas e torres) | **defesa** + chefe | Mítica | fixo 128×128 "Golfo da Argólida" | 30–35 | ✅ | — (usa G3, G4 ✅, G6 ✅, G8 ✅, G9 ✅ e G13 ✅) |
| 9 | `m9_tenaro` *(produzida)* | A Descida ao Tênaro / *The Descent at Taenarum* | **Criaturas míticas, jogando como Hades** (cobra: heróis contra míticas) | resgate + cerco leve | Heroica | gen M lakes, 9909 | 25–30 | — | — (usa G3, G4, G6 `maxAge` e `forbid`, G8, G10 e G11 `powerUsed` ✅) |
| 10 | `m10_otris` *(produzida)* | O Cerco de Ótris / *The Siege of Othrys* | **Cerco** (helépoles, muralhas concêntricas) + **Rei da Colina** no altar | **cerco** + **rei da colina** | Mítica | fixo 144×144 "Monte Ótris" | 30–35 | — | — (usa G2 `koth`, G3, G4, G5, G6, G8, G9, G13 e G17 ✅) |
| 11 | `m11_chamas` *(produzida)* | Argos em Chamas / *Argos in Flames* | **Êxodo + poderes divinos um a um** (cobra: proteger o líder) | **regicídio** (proteger o arconte) + escolta em massa | Mítica | fixo 128×128 "Argólida em Chamas" | 18 (fixo) | — | — (usa G1, G3, G4, G6 `remove`/`forbid`/`maxAge`, G8, G9 e G11 `powerUsed`/`reset` ✅) |
| 12 | `m12_titanomaquia` *(produzida)* | O Fim da Idade de Ouro / *The End of the Golden Age* | **Batalha final 3 × 1**: altares, depois Cronos em três idades, com Titãs (cobra: tudo) | **cerco** (Altares e Trono) + chefe em fases | Mítica | fixo 144×144 "Planície da Tessália" | 35–40 | — | — (usa G3, G4 ✅, G6 ✅, G9 ✅ e G13 ✅) |
...
- **Outras mudanças de ritmo do roteiro** (mesma correção, só medidas; o desenho das missões não mudou): m1 de 10m34s / 12m23s / 10m46s para 14m39s / 14m26s / 15m55s; m2 Fácil de 14m44s para 19m33s, e o secundário `fortress` deixou de ser cumprido pelo roteiro no Fácil e no Normal; m6 Normal/Difícil de 22m36s / 24m35s para 20m02s / 22m17s. Todas continuam dentro da janela.
- **m10 ficou mais fácil com a IA que não mira alvos invulneráveis** (correção trazida pela m12: `chooseAttackTarget`, a onda em curso e o alvo aliado ignoram edifícios com piso de vida 1). Na m10, o Portão de Bronze, os anéis e os Pilares só cedem com máquinas de cerco de Argos por perto (piso 1 sem elas); antes, a IA de Hades batia no Portão selado com a legião inteira, sem efeito. Agora a legião ajuda, e o roteiro vence aos 20m56s / 35m40s / 28m19s (antes 24m36s / 40m19s / 40m25s; confirmado desligando só `invulnerable()`). A janela do teste foi revista de propósito para 20m–45m30s, em vez de frear o roteiro.
...
| regicídio | m11 |
| rei da colina | m10 |
| relíquias | m9, secundário, com [G10] ✅ |
...
- **Com [G12] ✅** (não aplicado; muda o equilíbrio medido): Aquiles usa a Fúria ao chegar a cada aldeia, no Difícil — um gatilho `repeat` por aldeia com `{ "all": [ { "difficulty": "hard" }, { "units": { "player": 2, "tag": "aquiles", "near": { "point": { "entity": { "tag": "aldeia1" } }, "radius": 6 } }, "gte": 1 } ] }` e `{ "do": "ability", "unit": { "tag": "aquiles" }, "target": { "tag": "aldeia1" } }` (a recarga de 50 s é a do comando; o `target` só vale quando a Fúria sai).
...
- Aquiles não usa a Fúria no Difícil: [G12] ✅ já permite (acima, "Dificuldade"), mas ligar muda os tempos medidos e fica para a revisão com o dono.
...
**Produção (✅ feita):** `src/core/scenario/missions/m8_oceano.scenario.json`, registrada em `CAMPAIGN` (fecha o Ato II, depois da m7) e com roteiro e uma variante em `MISSION_SCRIPTS`. O mapa fixo "Golfo da Argólida" (128×128) sai de `scripts/maps/m8_oceano.ts` (primitivas de relevo com ruído de semente fixa 8808, aplicadas com as operações do editor `src/editor/ops.ts` e salvas por `saveMap`; `--write` embute em `map.data`, sem argumentos confere que o embutido é idêntico ao gerado; `--out` grava o `.map.json` avulso). Identidade: Argos no platô da Lárissa ao norte (CC centrado em [64,20]), fechado pelos braços oeste e leste e, ao sul, pela crista da Áspis (linhas 36–40) com três passagens — o Portão de Lerna (x 49–51: muro, portão `portao_oeste`, muro), a brecha da Via do Golfo (x 62–66: só os muros das pontas, aberta no meio) e o Portão do Istmo (x 77–79: muro, `portao_leste`, muro); os 6 muros formam o grupo `muralha` ([G5]); uma torre sobre cada portão (`torre_oeste`, `torre_leste`); Templo, Academia, Quartel, Estábulo, Oficina de Cerco e 4 Casas; 20 cidadãos. Micenas no vale noroeste [20,24], a Liga no vale do Aracneu a leste [110,64] (duas passagens para oeste), a planície no centro com os pântanos de Lerna (areia e três poças) e o ouro de Lerna junto da Hidra, duas estradas dos portões à praia, a praia larga de areia (linhas ~112–123) e o golfo a partir da linha 124. `validateMap` sem erros (3 avisos `noBase` nos inícios da Liga, de Oceano e de Micenas, que ganham o kit pela `config` ou não têm cidade). Como na m5, o `npm run map:check` do `.map.json` avulso não se aplica: ele joga o mapa como partida livre, sem o kit que a missão dá à Liga e a Micenas, e acusa as IAs sem base (a cidade de Argos tem ouro a 18 tiles do CC, dentro da muralha, fora do raio 16 da tabela). Testes próprios em `tests/m8_oceano.test.ts` (reprodutibilidade do mapa, entidades e tags, crista que só se atravessa pelas passagens, praia e golfo, setup, Portal só para Argos, as três marés, a 1ª maré que recua pelo tempo e a cura do Difícil, o Titã em ataque-movimento desde a subida e derrubando o CC parado na 2ª maré, as ondas do lado de fora com as passagens muradas, o reforço de Micenas, segredo de Lerna só por Argos, Jasão antes e depois da onda, HUD em PT e EN, janela do roteiro). O que mudou em relação à ficha:
- **Três marés, não uma luta só ([G9], [G4]).** Com o JSON da ficha (uma luta só), Oceano sobe aos 10 min diante de ~80 militares e dos quatro heróis da IA "difícil" jogando por Argos; medido nas marés, ela lhe tira um terço da vida em ~40 s, e com o Raio a luta inteira acabaria perto dos 11–12 min (estimativa), longe dos 30–35 min da §4. Agora ele vem em **três marés**: na 1ª (10 min) marcha em ataque-movimento até a crista (alvo: a brecha, [64,38]) com piso de vida de 2/3 (`hpFloor` 0,66); ao chegar ao piso — ou 150 s depois de subir —, **recua**: piso 1 (nada o fere), volta à praia em `move` e a contagem "A maré volta em" aparece no painel (`countdown { fromVar: "t_recuo" }`, 6 min). Na 2ª maré (piso 1/3) e na 3ª (sem piso) o alvo é o Centro Cívico, em ataque-movimento (`empurra` reordena quem ficou parado). **A coleira só vale depois que ele chega** (revisão): o gatilho `chegou` põe `perto = 1` quando ele entra a 8 tiles da brecha (1ª maré) ou a 30 do CC (2ª e 3ª); só então `volta1` (em `move`, de volta à brecha) e `volta` (em ataque-movimento, de volta ao CC) o puxam, e cada `ergue` zera `perto`. Antes, a coleira valia desde a subida (ele nasce a 80 tiles da crista) e trocava o ataque-movimento por `move` a cada 2 s (10 s nas marés 2 e 3): Oceano subia as três marés sem atacar (no roteiro do Normal, 1 s em `attack` na 1ª maré e nenhum na 2ª), e a vitória saía com o chefe praticamente parado. **O Raio e o piso:** o Raio de Zeus tira metade da vida máxima, mas só até o piso da maré. Usado no começo de uma das duas primeiras marés, ele a encerra na hora (tira 0,34 na 1ª e ao menos 0,33 na 2ª); usado no fim, quase nada. Na 3ª, sem piso e com no máximo 1/3 da vida (0,43 no Difícil), ele basta para derrubá-lo. Guardá-lo para a última não rende mais que usá-lo no começo de outra maré: a dica 3 ("use o raio no começo de uma maré, não no fim dela"), Zeus ("se o mar deixar") e a Pítia no 2º recuo ("se o raio de Zeus ainda estiver na mão, ele basta") dizem isso. A barra "🌊 Maré 1/3" mostra em qual maré se está. O objetivo virou "Derrote Oceano em três marés", e a vitória continua sendo Oceano morto (`oceano` só se cumpre depois de `ergue`; os pisos impedem a morte antes da 3ª).
- **[G3] sem mudar a hora da subida.** A ficha pedia Oceano aos 13/10/8 min. Com 13 no Fácil, a passiva passaria de 20 min; com 8 no Difícil, cairia antes dos 12 (a janela da §7.2 é 12–20). Oceano sobe aos 10 min nas três (o briefing e o objetivo dizem "dez minutos"); a dificuldade vem da IA da Liga (fácil/normal/difícil), das ondas e escoltas (`scaled`: o grupo escala como no `raid`) e, no Difícil, **o golfo cura 10 % da vida a cada volta** (`devolve2`/`devolve3`, com fala da Pítia). Na 3ª maré, essa cura só pesa para quem já gastou o Raio: com ele na mão, 0,43 − 0,5 < 0 e Oceano cai de uma vez. Fica assim: ela pune quem gastou o Raio antes, não quem o guardou. **Micenas cumpre a promessa de Atreu** (revisão): aos 30 s ("Mandamos o que temos"), 2 hoplitas e 2 toxotes chegam junto do CC de Argos nas três dificuldades e, no Fácil, mais 3 hoplitas, 2 toxotes e 2 hipeus (`atreu_facil`), sem mexer na hora da subida. Com isso a passiva perde na ordem das dificuldades (18m02s / 16m09s / 13m14s; antes o Fácil perdia antes do Normal, 15m47s × 18m53s).
- **A Liga começa com menos estoque** (500/500/300/50 Favor/200 Conhecimento; `set` no `setup`), no lugar dos 1500 de cada da `config` (que valem para todos os jogadores). Com o estoque cheio e a Idade Mítica, a IA Difícil da Liga arrasava uma Argos parada aos 10m56s (e aos 11m59s no Normal), antes de Oceano subir.
- **Micenas no Fácil:** a ficha pedia IA normal via `ai` [G16], que não existe. Um paliativo (Micenas com +1000 de cada no Fácil) foi medido e descartado: Micenas passava a enfrentar Oceano sozinha e a passiva do Fácil não perdia em 22 min. Micenas fica IA fácil nas três dificuldades; o alívio do Fácil é o reforço dela junto de Argos (acima).
- **Ondas e escoltas nascem na planície, sob a crista** (revisão). O `raid` contra o CC acha a origem pelo caminho até o alvo: com as três passagens muradas (o que a dica 4 e Jasão sugerem), nenhum tile a 30 tiles alcança o CC, a busca encurta a distância e, a 18 tiles, a espiral achava tiles do lado de dentro — as duas ondas nasciam na linha 37, dentro da muralha. Agora as ondas (e a escolta das marés 2 e 3) usam `spawn { scaled }` num ponto fixo, [65,51] (a origem de antes com as passagens abertas), e vão em ataque-movimento até a brecha ([64,38]); de lá, o gatilho `ondas_marcham` (a cada 5 s, quem está parado nas tags `onda1`, `onda2` e `escolta`) as manda ao Centro Cívico. Com tudo murado, o CC fica inalcançável e elas batem na muralha em vez de ficar paradas na planície (teste em `tests/m8_oceano.test.ts`). A escolta da maré (Hidra e 2 hipeus a cada 60 s, só na maré alta) vai à crista na 1ª maré (`mare_escolta1`, ainda um `raid` contra a brecha: a origem, a 30 tiles dela, fica fora mesmo com tudo murado) e ao Centro Cívico nas outras (`mare_escolta`); toda invasão contra o CC exige o CC de pé (sem invasão sem alvo).
- **Segredo `lerna`** com [G1] e [G13] (`kills` do jogador 0 do tipo `hydra`), sem o gatilho espelho da ficha: só vale a Hidra morta por Argos antes de Oceano subir (uma Hidra derrubada por Micenas não conta). A Hidra tem nome ([G8], "Hidra de Lerna") e guarda o ouro de Lerna. O prêmio (100 de Favor) é o da ficha.
- **Portal dos Titãs proibido à Liga e a Micenas** (`forbid` por jogador, [G6]): nenhum outro Oceano nem outro Prometeu. Argos pode chamar Prometeu.
- **Mapa:** o início 2 fica em [64,119] (a ficha pedia [64,120]; a margem mínima de um início é 8 tiles da borda) e Oceano sobe em [64,118]; a muralha parcial é a das três passagens da crista; entram 4 Casas (sem elas a cidade da ficha não abrigava os 20 cidadãos e o exército inicial).
- **Falas acrescentadas** (tom da §1.2, ≤ 200 caracteres): Jasão sobre a brecha, a Pítia com as teclas do Portal (V e B, mecânica nova) e explicando as marés, o recuo ("não o persiga até a praia") e a última maré ("o mar não o segura mais"); o Batedor nas duas ondas ("na planície, sob a crista"); Atreu avisando da Liga a leste; Oceano na 2ª e na 3ª maré (nesta, a pista de Lícaon da §1.1: "Um lobo me prometeu devolver o mundo aos Titãs"); Prometeu ao atender, apoiado na m4 e no "tu" dele ("Tu me soltaste do rochedo, Argos; agora eu atendo"); a Pítia se Jasão cair, antes da onda (`jasao_cai`) ou diante do mar (`jasao_cai_mar`); Rei Atreu (🦅, o ícone dele na m6) se a Liga cair; Jasão quando um portão cai e quando a Hidra morre. O `outro` da ficha foi dividido em dois parágrafos, sem mudar o texto.
- **Revisão dos textos e da coerência:** a intro diz "não mandou só homens" (a Liga ataca por terra); o objetivo da Liga diz onde ela fica ("vale do Aracneu, a leste") e uma dica nova a apresenta; o objetivo de Jasão virou "Jasão sobrevive às marés"; a fala da cura do Difícil diz "fechou parte das feridas"; a dica 2 não cita mais Aquiles, que morreu na m7, e ninguém o treina (`config.forbid: { "units": ["achilles"] }`); em EN, os nomes da interface ("Titan Gate", "Zeus's Bolt", "mythic creatures").
...
- **Lacunas:** nenhuma. Relíquias como secundário: [G10] ✅ (`map.relics` e `{ "stat": "relics" }`).
...
- **Relíquias** (secundário, com [G10] ✅): *"Recupere o Elmo das Trevas"*, com `{ "value": { "stat": "relics", "player": 0 }, "gte": 2 }` (relíquias guardadas agora nos Templos de Hades). Sem lista, o mapa gerado sorteia as relíquias pela semente; para pôr o Elmo junto das fendas, `"map": { "gen": { … }, "relics": [[x, y], …] }` (tiles do mapa 9909; o lint gera o mapa e avisa a posição que cair em lago, montanha ou fora da região dos inícios, e o jogo a leva à terra mais próxima).

**Derrota:** `DERROTA_CC` (o Palácio de Hades).

**Produção (26/09/2026, `src/core/scenario/missions/m9_tenaro.scenario.json`) — o que mudou em relação ao JSON acima:**
- **Posições no mapa real 9909.** Início 0 (Hades) em (22,34), início 1 (Culto) em (90,34), início 2 (sem ninguém) em (56,96). O platô do Culto (x 73–97, y 22–43) tem **uma só entrada por terra**, uma passagem de 3 tiles entre a serra e a mata (x 69–75, y 36–38); a faixa norte (y 12–21) e a terra do sudeste se alcançam por outros caminhos. Pelos deslocamentos da ficha, `jaula1`, `jaula2`, `fenda1` e `fenda2` caíam em bolsões inalcançáveis (clareiras da mata e a ponta leste depois do lago), e a 1ª tentativa de `fenda1` fechava a entrada do platô. Ficou assim (relativo ao início 1): `jaula1` (−10,+6), na borda sudoeste do platô; `jaula2` (+5,−5), junto ao Centro Cívico do Culto; `jaula3` (0,−14), na faixa norte (igual à ficha); `fenda1` (−32,+6), a oeste da entrada do platô; `fenda2` (+10,+28), além do lago do sul; `fenda3` (+12,−18), na ponta leste da faixa norte. Todas conferidas com `rectReachable` a partir do Palácio (a pé: 38 a 160 tiles). Os Ciclopes nascem no ponto da própria jaula.
- **Ouro do Culto (paliativo do mapa).** A semente 9909 não põe ouro a menos de 26 tiles do início 1: a IA do Culto erguia minas junto ao Centro Cívico, mandava 20 cidadãos atravessar o mapa e não treinava **nenhum** soldado em 14 min. O Culto ganha **+1000 de ouro no `setup`** e **+100 por minuto** enquanto estiver de pé (gatilho `tesouro`); com isso chega à Mítica aos ~9–10 min, ergue Templo e exército e é eliminado pelo roteiro aos ~21 min no Fácil e no Normal e só no fim (~32 min) no Difícil. Nem a IA nem os dados mudaram.
- **Idade máxima Mítica** (`config.maxAge: 3`, G6) no lugar de `forbid: titan_gate`: o Titã de Hades seria o próprio Cronos, e sem o teto as duas IAs guardavam ouro para a Idade dos Titãs (o Culto a alcançava aos ~16 min) em vez de treinar.
- **Dificuldade com G3:** fendas a cada 150/120/90 s e Perseu aos 60/90/150 s, como a ficha pede. `every` é o resto da divisão do relógio, então os `after` 180/240/300 da ficha não escalonam as fendas: elas pulsam juntas (no Normal, `fenda1` e `fenda2` aos 240 s e as três a partir dos 360 s).
- **Relíquias (G10):** `map.relics` = [97,66] (perto da `fenda2`), [55,44] (perto da `fenda1`) e [95,13] (perto da `fenda3`); objetivo secundário `elmo` = 2 relíquias guardadas. Lint sem avisos (terra alcançável).
- **Segredo `sombras` com G1** (`done` direto, sem gatilho espelho); o gatilho `sombras_legiao` dá os 150 de Favor quando ele se cumpre.
- **Nomes (G8):** Jaula de Brontes / Estéropes / Arges e Fenda do Estige / Aqueronte / Flegetonte; facções `{ pt, en }` (*Legions of Hades*, *Cult of Cronus*, *Jailers of Cronus*).
- **HUD (G4):** contadores "⛓️ Jaulas abertas n/3" e "🌀 Fendas fechadas n/3" (`hud.progress { var, format: count }`).
- **Falas novas** (além das da ficha): a Pítia com a tecla X do Cérbero (75 s); uma **Sombra batedora** (👻) na 1ª onda das fendas; Hades quando o Culto usa o **raio roubado** (`powerUsed`, G11 — a IA do Culto o gasta em Héracles ou Perseu no começo, e a fala diz que ele só cai uma vez); Arges com as três jaulas abertas; Hades ao recuperar o Elmo das Trevas ("foi com ele que Lícaon se escondeu entre os meus sacerdotes"); Hades no prêmio das Sombras. A fala da anagnórise (`pacto`, "Lícaon me usou como um manto") virou o 1º parágrafo do `outro`. Dicas (`hints`, 7) sobre o Palácio, o triângulo (com as míticas que o Culto de fato usa), o raio do Culto, as Sombras, as jaulas (com o lugar de cada uma), as fendas (com o lugar e o Oráculo de Apolo) e o Elmo.
- **Passiva:** derrota aos 13m04s / 11m42s / 8m50s (o rascunho media 15/10/8; a 1ª produção, com as ondas nascendo dentro da cidade, 13m39s / 11m25s / 7m36s).
- **Revisão (26/09/2026), o que mudou depois da 1ª produção:**
  - **Elenco:** o Templo da IA treina primeiro `perseus`, `achilles`, `heracles`, `odysseus` e `jason` (`ai.ts`), e o Culto punha Jasão (morto na m8), Odisseu (em dívida com Argos) e Aquiles (morto na m7) contra o jogador. O Culto tem `forbid.units` com os cinco heróis (G6 por jogador) e o jogador 0 não treina `jason` nem `achilles`.
  - **Míticas do Culto** (a mecânica cobrada): com a Heroica pronta ele começava sem deuses menores e gastava o Favor em heróis, e o Pégaso da ficha é batedor desarmado (a IA o ignora). Agora `set minorGods: ["hermes", "dionysus"]` no `setup` (Centauros e Hidras, poderes Cessar-fogo e Pele de Bronze; na Mítica a IA ainda escolhe Hera ou Ártemis) e o Centauro vem **primeiro** na onda da `fenda3` (sobrevive ao corte do Fácil). Medido no roteiro: Centauros das fendas nas três dificuldades a partir de 5–6 min; Medusa, Centauro e Hidra do Culto no Normal e no Difícil (no Fácil o Culto não chega a treinar míticas). A dica do triângulo cita essas criaturas.
  - **Ondas das fendas:** o `raid` com `angle` 3/1/5 da ficha nascia a oeste e dentro da cidade (a origem do ângulo 5 recuava até 5–6 tiles do Palácio, com o petróbolo já ao alcance dele), e os ângulos 0/7/1 põem a origem em cima das minas de ouro do jogador, a 22 tiles do Palácio (com o Culto já sem heróis, o Difícil do roteiro perdia aos 14–16 min com `distance` de 22 a 34). Cada onda agora **nasce na própria fenda** (`spawn` com `scaled` + `order attackMove` até `{ tc: 0 }`, o padrão de "desligar a fonte" da m5): chegam ao Palácio em ~12 s (Estige), ~35 s (Flegetonte, pela faixa norte) e ~35 s (Aqueronte, cavalaria). Por não serem `raid`, o harness não as conta em "invasões" (0/0).
  - **Textos com o onde:** objetivos `jaulas` ("duas no platô do Culto, a leste, e uma na faixa norte, acima dele") e `fendas` ("entre as duas cidades, no extremo nordeste e a sudeste, além do lago"); dicas com a jaula de cada Ciclope (a de Estéropes junto ao Centro Cívico do Culto: é preciso invadir a cidade dele) e o Oráculo para achar as fendas.
  - **Raio roubado:** a IA lança o Raio no herói ou mítica de maior `maxHp` (×1,5 se herói) a até 40 tiles do Centro Cívico dela, sem olhar névoa nem escolta; a dica agora diz isso e manda um Cérbero na frente com os heróis longe (a Fenda do Estige e a relíquia [55,44] já estão ao alcance).
  - **Elmo:** as três relíquias são iguais; a dica e o objetivo dizem que o Elmo foi partido e que duas relíquias (pedaços) nos Templos o recompõem.
  - **Tom (§1.2):** o aviso das ondas saiu de Hades para a Sombra batedora. Hades fala no início, no raio roubado (virada), no Elmo (a pista de como Lícaon se escondeu) e no prêmio das Sombras: exceção registrada por ele ser o patrono do jogador nesta missão, como Zeus nas de Argos; a fala final está no `outro`.
  - **Derrota:** o `setup` guarda o Palácio (`storeEntity` `#palacio` = `{ tc: 0 }`) e a derrota é `{ entity: { tag: "palacio" }, exists: false }`, como a dica diz ("o Centro Cívico inicial"); antes, com uma 2ª cidade, perder o Palácio não encerrava a missão.
...
**Produção (✅ feita, 26/09/2026):** `src/core/scenario/missions/m10_otris.scenario.json`, registrada em `CAMPAIGN` (Ato III, depois da m9) e com roteiro em `MISSION_SCRIPTS`; testes próprios em `tests/m10_otris.test.ts`. O mapa fixo "Monte Ótris" (144×144) sai de `scripts/maps/m10_otris.ts` (primitivas de relevo com ruído de semente fixa 1010, aplicadas com as operações do editor `src/editor/ops.ts` e salvas por `saveMap`; `--write` embute em `map.data`, sem argumentos confere que o embutido é idêntico ao gerado; `--out` grava o `.map.json` avulso). Identidade: ao norte, o platô do Ótris, fechado pelas escarpas oeste (x ≤ 37), leste (x ≥ 107) e sul (linhas 51–55); a escarpa sul só se sobe pela rampa do meio (x 70–74), fechada pelo **anel externo** (4 muros, grupo `muralha_externa` [G5], e o Portão de Bronze no vão [72,52]); dentro, o pátio da cidadela com o CC do Culto centrado no início 2 [72,32], Templo, 2 Quartéis, Estábulo, Academia, Oficina de Cerco, 8 Casas, 6 torres (4 atrás do Portão de Bronze, 2 junto ao portão interno) e 15 cidadãos, com ouro e bosques nos cantos; acima dele, o santuário num terraço (x 50–94, linhas 3–26) fechado por cristas e pelo **anel interno** na crista sul (4 muros `muralha_interna` e o portão `portao_interno` em [72,28]); no santuário, os três Pilares (Fortalezas do Culto, `pilar1` [60,24], `pilar2` [84,24], `pilar3` [72,14]). No meio da planície, o Altar do Tempo (`koth: [72,80]`): uma clareira de terra batida num lajedo de areia entre **seis pedras eretas**; a Via dos Titãs sobe do altar à rampa; Argos a sudoeste [24,120] (com a Oficina de Cerco do mapa) e Hades a sudeste [120,120], ligados ao altar por estradas; morros, bosques e ouro disputado na planície e nos flancos do Ótris. `validateMap` sem erros (avisos: `noBase` no início de Hades e no das Sentinelas, que só têm o que o cenário cria, e 3 `chokepoint`). Números medidos depois da revisão (`npx tsx scripts/missions.ts 26 m10_otris`): passiva **em jogo aos 26 min no Fácil** e com **derrota aos 18m57s / 14m32s** no Normal / Difícil (4 / 3 / 2 sortidas contra o acampamento, todas com unidades) e roteiro com **vitória aos 24m36s / 40m19s / 40m25s** (Fácil / Normal / Difícil) na janela estrita 21m–45m30s, com 4 / 7 / 7 sortidas, todas contra o altar tomado; com a passiva padrão de 14 min, em jogo nas três. O que mudou em relação ao JSON da ficha, e por quê:

- **O Culto se trancou de verdade.** O Portão de Bronze é um bloco de muralha **lacrado** (`place` exato de `wall` com `tag` `portao_bronze` e nome "Portão de Bronze"/"Bronze Gate" [G8]), não um `gate`: nem o Culto passa. Com um portão de verdade, a IA difícil do Culto, com a Mítica e o estoque da ficha, saía em ondas contra a Argos parada e a passiva perdia aos 6–10 min, longe dos 15–25 da §7.2; com o estoque cortado (medido 1000/1000/800 e 800/800/300), ela ainda saía aos 8–9 min, e no Rei da Colina o exército dela ia pingando até o altar e morrendo, sem defender a cidadela (o roteiro vencia aos 12–13 min). Trancado, o Culto defende a cidadela com o exército que junta lá dentro (e com o estoque da ficha, 2000/2000/1500); quem ataca Argos são as sortidas das Sentinelas. É também o que a intro diz: "O Culto se trancou no Monte Ótris". O vão do portão fica aberto no mapa avulso (o cenário o fecha no `setup`).
- **A pedra do Ótris só cede às máquinas de Argos** ([G9] `hpFloor`). Muros, portões e Pilares nascem com piso de vida 1 (não sofrem dano) e o perdem enquanto houver uma helépole ou um petróbolo de Argos a até 12 tiles (`bronze_mortal`/`bronze_divino`, `anel_mortal`/`anel_divino`, `pilarN_mortal`/`pilarN_divino`, com as variáveis `ext_mortal`, `int_mortal` e `pN_mortal` para só agir nas trocas). Sem isso, as ondas da IA de Hades (normal, igual nas três dificuldades) derrubavam o Portão de Bronze aos 5 min e, sozinhas, venciam a missão na passiva aos 22m43s. Com as máquinas por perto, todo golpe conta, inclusive os de Hades. A 1ª versão exigia só "um militar de Argos" por perto; com ela, qualquer soldado que perseguisse um inimigo até a rampa abria o portão para as legiões de Hades. Na revisão, os Pilares passaram à mesma regra (antes aceitavam qualquer militar de Argos, contra a dica), e os Pilares 1 e 2 só cedem **depois do anel interno rompido** (portão interno caído ou um muro dele): do pátio, por cima da crista, uma helépole ou um petróbolo alcançavam os dois. O objetivo `portao` e a fala de Lícaon (`portao_cai`) valem para o anel externo rompido por qualquer peça, como o `interno_cai` (o Portão e os 4 muros são blocos iguais na tela; antes, quem entrava por um muro vencia com "Rompa o Portão de Bronze" pendente); o texto diz "Rompa o Portão de Bronze (ou um muro ao lado dele)". Cada muro tem o seu piso (`pick: "nearest"` num ponto do centro do tile, [70,5; 52,5] etc.: num ponto de canto, dois muros vizinhos empatavam e dois ficavam sem piso — foi por um deles que o Culto saiu numa das medições). Hades avisa uma vez (`hades_bronze`: "traga as suas máquinas").
- **As colmeias dos Pilares** ([G9] `hp`): "três Pilares que zumbem como colmeias". Com 3/4, 1/2 e 1/4 da vida, cada Pilar solta um enxame de 8 Sentinelas (mirmidões, arqueiros cretenses, um Cérbero e uma Medusa; `colmeiaNa/b/c`, tags `enxameNa/b/c`), e a pedra não cede enquanto o enxame viver. Sem elas, com a pedra cedendo, 50–60 militares derrubavam cada Pilar em 15–25 s.
- **Guarnição das Sentinelas.** No santuário, duas guardas de 16 (mirmidões, hipaspistas, arqueiros cretenses, Medusas e um Cérbero) e um reforço de 6 a cada 90 s a partir dos 4 min, **no máximo 12 vezes** (`repeat: { max: 12 }`, [G17]): sem teto, o reforço empatava com o cerco e o roteiro ficava em jogo até os 55 min. No altar, uma guarda de 7 (hipaspistas, arqueiros e um Cérbero, `scaled` [G3]): trancado, o Culto não disputa a colina no começo. Em geral as legiões de Hades a matam antes de Argos chegar (aos ~45 s nas três dificuldades): a Pítia e a dica não prometem mais "as Sentinelas que o guardam"; quem disputa o altar são as sortidas (abaixo). **Ciclopes:** as Sentinelas não têm mais Ciclope (guarda, enxames e sortidas usam o Cérbero, mítica do deus do Culto): pela sinopse (§1.1) e pelo `outro` da m9, os Ciclopes estão do lado de Argos e Hades.
- **O altar exige Argos nele** ([G2] `koth`): a conta sobe quando a colina é do time de Argos (sem inimigos a 6 tiles, a regra do modo) **e** há um militar de Argos nela. Com o texto da ficha (Argos ou Hades), as legiões de Hades tomavam o altar aos 3 min sozinhas, a passiva cortava as sortidas e nunca perdia. A barra do painel mostra a conta ([G4], formato tempo). Em cenário, a barra do topo não mostra a posse da colina (ela conta o time inteiro, e "Argos & Hades 137s" com o altar em 0:00 enganava), o evento de posse diz quem está de fato nela, sem verbo no singular ("⛰ Colina tomada: Hades.", `ev.kothTakenScenario`, `src/core/sim/modes.ts`), e o aviso de início do modo ("segure a colina por 4 minutos") não aparece (`src/main.ts`).
- **Sortidas e o altar disputado** ([G3]): a composição da ficha com um Cérbero no lugar do Ciclope (3 hipaspistas, Cérbero, Medusa, Cérbero, em `raid` contra o CC de Argos, ângulo 7), hipaspistas primeiro — o `raid` do Fácil fica com os 4 primeiros e o do Difícil soma a primeira metade (4 / 6 / 9). **A mesma cadência nas três dificuldades**, a cada 4 min 30 s desde os 9 (`sortida`); o tamanho é que escala. Na 1ª versão o Fácil apanhava primeiro e mais vezes (a cada 3 min desde os 6, contra 5 min 30 s desde os 11 no Difícil: até os 15 min, 16 unidades no Fácil e 9 no Difícil). **Tomado o altar, as sortidas não acabam: descem da rampa contra ele** (`sortida_altar`, `raid` com alvo [72,80] e ângulo 6, uma retomada menor — 2 hipaspistas, Cérbero e Medusa: 3 / 4 / 6), e **se as Sentinelas segurarem a colina por 3 s** (`altar_perdido`: `koth` do time 1), o objetivo `altar` volta a pendente (`{ "do": "objective", "status": "pending" }`), a conta zera, a barra volta ao painel e as sortidas voltam ao acampamento até Argos segurar o altar mais 2 min. Na 1ª versão o altar era cumprido aos ~2m30s com um clique (Hades já tinha limpado a guarda) e nenhuma sortida saía depois disso: Argos não podia mais perder e o roteiro nunca enfrentava uma sortida (a alavanca G3 não se aplicava). A retomada com a sortida inteira (9 no Difícil, a cada 4 min 30 s) levava o roteiro do Difícil a 45m13s–45m23s, na beira da janela (sem ela, 38m27s): ela é menor que a sortida contra o acampamento. O Batedor avisa a 1ª retomada e cada perda do altar; a fala de Hades ao tomá-lo diz que as sortidas agora descem contra ele. A variação de altar da ficha (90/120/180 s) e o "kill no pilar3 no Fácil" continuam de fora (o objetivo diz "2 minutos"), assim como `scaled` na guarda do santuário e no reforço: o Difícil já fica a 5 min do teto da janela.
- **Hades trouxe as legiões dele e os Ciclopes** (intro: "Hades trouxe suas legiões e os Ciclopes que libertou do Tártaro"): 4 hipaspistas, 4 arqueiros cretenses, 2 Cérberos e 2 Ciclopes no `setup` (`legiao`). Só com o kit, a IA do Culto (antes de trancado) arrasava Hades aos 4 min 30 s.
- **Lícaon** nasce com nome ([G8]) e **coleira** (`licaon_coleira`: a cada 4 s, se estiver a mais de 10 tiles do santuário, volta a [72,20]); numa medição ele saiu atrás de um batedor e morreu para Hades. O segredo agora exige **Argos** o matar ([G13] `kills` do tipo `nemean_lion`), como a Hidra da m8, e dá 100 de Favor.
- **Mapa:** o início 3 das Sentinelas fica em [72,8] (a ficha pedia [72,6]; a margem mínima de um início é 8 tiles da borda). Os anéis são curtos (5 tiles, muro-muro-portão-muro-muro) porque as escarpas e as cristas fazem o resto do anel; o CC do Culto e os cidadãos são entidades do mapa (`startKit` [true, true, false, false], como na ficha).
- **Travas ([G6]):** Idade Mítica é o teto (`maxAge: 3`: nenhum Portal dos Titãs, nenhum Cronos antes da hora e nenhum Prometeu), ninguém treina Jasão nem Aquiles (`config.forbid`), e o Culto não treina Perseu, Héracles nem Odisseu.
- **Falas acrescentadas** (tom da §1.2, ≤ 200 caracteres): a Pítia apresentando o altar ("...mas só com soldados seus nele") e as teclas do cerco (W e Q, mecânica nova: "Helépoles abrem: a Oficina de Cerco as constrói") e explicando as colmeias; o Batedor na 1ª sortida, na 1ª retomada do altar e a cada perda dele; Hades pedindo o altar, ao tomá-lo, diante do bronze sem máquinas e se o acampamento dele cair; Hades quando o anel interno cede; Cronos no 1º e no 2º Pilar ("Três mil anos de espera... e agora só uma pedra"), com a Pítia ("Ele não teme as pedras que caem, arconte. Ele as conta."). **A peripécia é de Hades** ("Os pilares não protegiam meu pai, arconte. Prendiam-no."), não da Pítia como na ficha: o `outro`, logo depois, diz "A Pítia não diz nada. Não precisa.". O `outro` da ficha foi dividido em dois parágrafos, sem mudar o texto; os objetivos dizem onde fica cada coisa. As 8 dicas têm no máximo 200 caracteres (PT e EN; na 1ª versão, 6 de 7 passavam disso).
- **Placeholder visual:** os Pilares aparecem como Fortalezas (entidades do mapa não têm nome próprio; [G8] só vale para `spawn`/`place`). A dica diz que "os três Pilares do Tempo são as Fortalezas do santuário".
...
```json
{
  ...
  "triggers": [
    ...
    { "id": "prometeu_cai", "when": { "all": [ { "fired": "prometeu" }, { "entity": { "tag": "prometeu" }, "exists": false } ] }, "then": [
      { "do": "say", "speaker": { "pt": "Oráculo de Delfos", "en": "Oracle of Delphi" }, "icon": "🔮",
        "text": { "pt": "O Tempo derruba Prometeu. Não olhe para trás, arconte.", "en": "Time fells Prometheus. Don't look back, archon." } } ] }
  ],
...
- **2º Raio** ([G11] ✅): a ira de Zeus pode repor o Raio depois do 1º uso — `{ "id": "raio2", "when": { "all": [ { "time": { "gte": 330 } }, { "powerUsed": { "player": 0, "id": "bolt" } } ] }, "then": [ { "do": "set", "player": 0, "powers": { "reset": ["bolt"] } }, { "do": "say", … } ] }`. `powerUsed` conta usos desde o início (`"gte": 2` = o 2º Raio já caiu), e cada onda pode tirar o poder que não serve mais (`"remove"`).
...
**Produção (`src/core/scenario/missions/m11_chamas.scenario.json`, mapa por `scripts/maps/m11_chamas.ts`, testes em `tests/m11_chamas.test.ts`).** O que mudou da ficha, e por quê:
- **Mapa "Argólida em Chamas"** (4º mapa fixo, gerado de forma reprodutível pelas operações do editor e embutido em `map.data`): os montes do norte com a **Descida de Cronos** (desfiladeiro de terra batida, x 59–69); **Argos inteira** na planície sob o rochedo da Lárissa (CC, Templo com tag `templo`, Academia, Quartel, Estábulo, Mercado, Fortaleza voltada ao Culto, 10 Casas, a muralha norte — grupo `muralha`, G5 — com o portão `portao` e as torres `torre_oeste`/`torre_leste`; 40 cidadãos, 8 hoplitas, 6 arqueiros cretenses e 4 hipeus); o vale do Culto a nordeste, fechado pelo Aracneu; a **Via de Náuplia** por **Tirinto em ruínas** (torre `tirinto` em [40,80] num anel quebrado de muralha ciclópica); o **rio Ínaco** com **dois vaus** (o do sul na Via, o do norte levando à margem oeste, caminho mais longo); o **cais de Náuplia** (areia, a água do golfo na linha 116) e os pântanos de Lerna. O início 2 ficou em **[64,8]** (a ficha pedia [64,4]; a margem mínima de um início é 8 tiles). Os olivais perto da cidade foram necessários: sem eles, os lenhadores subiam até a boca do vale do Culto e morriam lá. `validateMap` sem erros (3 avisos: `noBase` nos inícios do Culto, que ganha o kit pela `config`, e de Cronos, e um gargalo no fundo do vale do Culto); como na m5 e na m8, o `npm run map:check` do `.map.json` avulso não se aplica (joga o mapa como partida livre, sem o kit da missão, e acusa as IAs sem base).
- **As naus** são postas pelo `setup` (`place`, exato em [18,112]) para ganhar nome (G8: "Naus de Micenas") e **piso de vida de 30 %** (G9): um ataque ao cais não afunda a missão. O embarque usa **`remove`** (G6) e **raio 5**, não 4: 30 cidadãos mandados ao Mercado ficam 28 a ≤ 4 tiles do centro dele e 30 a ≤ 5 (os empurrados para fora esperariam para sempre); só cidadãos embarcam (o objetivo e a dica dizem isso), e só até os 18 min (`time < 1080`: antes, com meta − 1 a bordo, o embarque do mesmo segundo completava a meta depois de o objetivo falhar e a partida ficava sem vitória nem derrota). O segredo `chama` (oculto, G1) é concluído pelo gatilho `chama_salva`, posto **antes** do `embarque`: com o sacerdote a ≤ 5 tiles das naus **e um lugar livre numa nau atracada**, ele sobe a bordo na hora (`remove` + 1 embarcado) e a Pítia fala da brasa. Na 1ª versão o segredo valia com o sacerdote no cais vazio ("sobe a bordo" minutos antes da 1ª nau); um `done` com `fired nau1` não basta, porque o `embarque` do mesmo segundo em que a nau atraca pode remover o sacerdote antes de o objetivo ver a nau (medido: sozinho no cais, o segredo se perdia). As naus não afundam em combate, mas o jogador pode demolir as próprias naus (Del ou o botão Demolir ignoram o piso): isso falha o `exodo` na hora, com uma fala, e a derrota segue o objetivo (`{ "objective": "exodo", "is": "failed" }`).
- **Dificuldades (G3), como a ficha sugeria:** meta **20 / 30 / 40**; naus de **7-7-6** no Fácil (senão a meta de 20 se cumpria aos 9 min, abaixo da janela), **10-10-10** no Normal e **10-10-10-10** no Difícil, com a 4ª aos **14 min** (aos 15, o Culto "muito difícil" chegava ao cais antes do último embarque e o arconte caía aos 15m21s); **Cronos aos 12,5 / 12 / 10 min** e Prometeu 1 min antes (a 1ª versão punha Cronos aos 14 min no Fácil, depois da última nau dos 13 min: a vitória saía aos 13m11s sem Cronos, Prometeu contra ele, Atena e a Restauração — o êxodo não pode terminar antes do clímax); as ondas e a vanguarda escalam (`raid` e `spawn { scaled }`). O painel mostra as contagens das naus e de Cronos da dificuldade e as barras "A bordo" e "Lugares nas naus atracadas" (G4). Por isso o objetivo `exodo` não traz número ("Embarque os cidadãos nas naus de Náuplia antes que zarpem") e a 2ª fala da intro diz "cada vida salva", não "trinta".
- **As ondas vêm do jogador 2** (a marionete Cronos), não do Culto (IA): a IA absorveria as unidades do `raid` no exército dela. A 3ª onda sai do nordeste (ângulo 7, pela Via) porque o sudeste do cais aqui é o golfo. Quem sobra de uma onda, parado, **vai caçar o arconte** (gatilho `hostes`) — é a cobrança de "proteger o líder". A cavalaria da 3ª onda mira as naus, e as naus no piso de vida nunca morrem: sem mais nada, quem ataca um edifício só o larga quando ele cai, e a onda ficava batendo nelas para sempre (nem voltava aos cidadãos que chegavam ao cais, nem ficava parada para o `hostes`). Com as naus no piso, `cais_fila` manda quem do jogador 2 estiver a ≤ 6 tiles delas atacar o cidadão mais perto das naus (com gente a ≤ 8) e `cais_arconte`, sem ninguém na fila, atacar o arconte (ordens diretas: um ataque-movimento readquiria as naus). Mirar um ponto do cais em vez das naus não resolve (medido: a onda chega, mata a fila e trava nas naus do mesmo jeito); mudar o motor (não adquirir edifício no piso) mexeria no cerco da m10 e nos altares da m12. Os poderes vêm por `set minorGods` (Afrodite, Hera, Hermes; Atena com Cronos), como na ficha, e o **2º Raio** (G11 `powerUsed` + `reset`) volta uma vez depois dos 5,5 min.
- **Acréscimos:** a **vanguarda de Lícaon** (7,5 min, 12 soldados com 2 catapultas, escalada, descendo o desfiladeiro do norte — a Descida, nome que o jogador não ouve — rumo ao portão norte; o vilão fala) e as pesquisas militares do Culto, para que algo além das três ondas bata na cidade antes de Cronos (a IA que joga no teste ainda a segura com folga; para quem joga, é mais uma frente enquanto organiza as levas); falas curtas da Pítia (a dica com o botão direito e o arconte — que não tem coroa no mapa, é um humanoide como os outros: a dica diz onde ele começa e que o painel o mostra com 👑 —, o primeiro a bordo, o arconte ferido, a queda do CC, os 2 minutos finais), do Batedor (as quatro naus, a vanguarda), de Zeus (o templo que arde, o 2º Raio) e de Atena (a Restauração).
- **Prometeu e Cronos:** Prometeu surge em [64,22], ao pé da Descida (a ficha usava o CC, que pode já ter caído), e recebe a ordem de atacar Cronos **1 s depois** (`prometeu_luta`): no disparo que invoca Cronos, a marionete ainda não conta como viva e a ordem de `attack` era descartada. Cronos tem **piso de 30 %** (G9: dano, Maldição e o Raio param nele), marcha para a cidade (ponto fixo [64,40]) e, parado depois dela, **desce a Via de Náuplia até Tirinto e fica lá** — a estrada fecha e a Pítia aponta o vau do norte. Ele não vai ao cais (o `outro` o faz recuar ao Ótris).
- **Vitória** = os objetivos `exodo` e `arconte` cumpridos (1 s depois da contagem; assim a tela de fim os mostra cumpridos). **Derrota** = o arconte cai ou o `exodo` falha (18 min sem a meta, ou as naus demolidas). **Tratamento:** todos falam com o arconte no singular e por "você" ("Encha o convés", "Você tem outro"); a queda de Prometeu é "O Tempo derruba Prometeu" (ele nunca esteve no Tártaro: vinha das correntes do Cáucaso, m4). **Travas (G6):** ninguém ergue o Portal dos Titãs (nada de 2º Prometeu nem 2º Cronos), Idade máxima Mítica, Jasão e Aquiles fora (m8, m7), o Culto sem heróis gregos.
- **Pendências:** o `outro` fala do sacerdote com a brasa mesmo sem o segredo `chama` ([G15], `outro` condicional). E uma do motor, fora do cenário: um cidadão que coletava guarda o nó antigo (`nodeId`) ao receber `move`, e `depleteNode` o manda de volta à coleta se esse nó se esgota enquanto ele caminha; no êxodo, alguns dos que foram mandados ao cais voltam a cortar lenha pelo caminho (a tecla S antes do clique evita). A correção muda a simulação de todas as missões (`SIM_VERSION`) e fica para uma rodada própria.
...
- **Lacunas:** ✅ [G6] (`config.players[i].maxAge` ou `forbid { "buildings": ["titan_gate"] }`) impede um 2º Cronos da IA, [G9] (`hp`, `hpFloor`, `damage`/`heal`) permite fases de vida de Cronos e [G13] (`kills … by`) sabe quem deu o golpe final.
...
```json
{
  ...
  "triggers": [
    { "id": "abertura", "when": { "time": { "gte": 1 } }, "then": [
      { "do": "say", "speaker": "Zeus", "icon": "⚡",
        "text": { "pt": "Irmãos. Da última vez que vocês estiveram juntos, eu os tirei da barriga dele.", "en": "Brothers. The last time you were all together, I pulled you out of his belly." } },
...
**Produção (✅ feita, 26/09/2026):** `src/core/scenario/missions/m12_titanomaquia.scenario.json`, registrada em `CAMPAIGN` (Ato III, a última do plano; nesta árvore vem logo depois da m9 — m10 e m11 entram antes dela quando forem produzidas) e com roteiro em `MISSION_SCRIPTS`. O mapa fixo "Planície da Tessália" (144×144) sai de `scripts/maps/m12_titanomaquia.ts` (primitivas de relevo com ruído de semente fixa 12012, aplicadas com as operações do editor `src/editor/ops.ts` e salvas por `saveMap`; `--write` embute em `map.data`, sem argumentos confere que o embutido é idêntico ao gerado; `--out` grava o `.map.json` avulso; `--ascii` imprime o relevo). Identidade: no **noroeste**, o pico do Olimpo e a escarpa que cerca a Nova Argos (início 0 = [24,24]) com três passagens — a leste para o Vale do Tempe, a rampa do sudeste para o Campo e a do sul para a estrada da caverna —, cada uma com uma torre de Argos do lado de dentro (`torre_leste`, `torre_rampa`, `torre_sul`); no **norte**, o Vale do Tempe e o esporão do **Ossa**, que desce da borda até o meio do mapa e separa o Olimpo do Ótris (do Trono à Nova Argos só pela planície, ~100 tiles); no **nordeste**, o pico do Ótris e o planalto do Culto (início 3 = [120,24]), fechado ao sul pela serra do Ótris com duas saídas (a Estrada do Trono, x ~96–104, com as torres `torre_trono1`/`torre_trono2`, e a trilha do leste) e o **Trono** (`trono`); no **sudoeste**, a caverna de Hades (início 1 = [24,120]), terra escura cercada de rocha, com a poça do Estige e duas bocas (norte e leste); no **sudeste**, a costa de Poseidon (início 2 = [120,120]), praia e mar (água intransponível, sem naval); a leste do Campo, o lago Bébeis; no **centro**, o Campo da Titanomaquia (terra batida) com o chão dos três Altares nos centros da ficha e estradas para os quatro cantos (os Altares e o Trono não são entidades do mapa: nascem do `setup`, com nome, [G8]). `validateMap` sem erros (2 avisos `noBase` nos inícios de Hades e Poseidon, que ganham o kit pela `config`); recursos a 16 tiles de cada início: Olimpo 1435 / 3900 / 3600 (comida / madeira / ouro), caverna 2130 / 2550 / 2700, costa 2130 / 2250 / 4500, Ótris 1570 / 1200 / 3600. Como na m5 e na m8, a parte de IA do `npm run map:check` não se aplica (o arquivo avulso não tem o kit que a missão dá a Hades, Poseidon e ao Culto). Testes próprios em `tests/m12_titanomaquia.test.ts` (reprodutibilidade do mapa, entidades e tags, as passagens do Olimpo, as saídas do Ótris e as bocas da caverna, o Ossa, o mar; setup; G6; o juramento e a vingança; o prazo e os Colossos; o teto de 2/3 com Altares de pé e o Trono intocável antes de Cronos; as três idades com o recuo ao Trono; a hora devorada sem Trono e a cura do Difícil; o segredo de Perseu; o HUD em PT e EN; o registro e a janela). O que mudou em relação à ficha:
- **Cronos sai do trono pelo relógio, não pelos Altares.** A ficha o soltava quando o último Altar caía (ou aos 25 min). Medido na 1ª versão: a IA do jogador derrubava os três Altares aos 6–8 min (os exércitos dos irmãos já brigavam neles) e vencia aos 11–16 min no Fácil e no Normal — abaixo da janela de 24m30s, e o fim dependia de quem batia primeiro. Uma 2ª versão (o Trono, exposto pelos Altares, soltava Cronos ao cair) ainda vencia aos 13–15 min. Agora Cronos sai sempre no prazo (30 / 25 / 20 min com G3; a contagem está no painel desde o 1º segundo) e os Altares decidem **com que força**: com um deles de pé, ele não passa de 2/3 da vida; sem eles, o devorar para e, quando ele sai, o Trono fica exposto. Quem termina cedo usa a espera (Prometeu pelo Portal, os irmãos, o Culto) — a Pítia diz isso quando o 3º Altar cai. Intro, dicas e falas foram ajustadas ("derrube os Altares antes, e ele sairá com fome e sem armadura").
- **O juramento do Estige (novo).** Os Altares e o Trono têm piso de vida 1 ([G9]) e só sofrem dano com **≥ 6 militares de Argos a até 14 tiles** (gatilhos `foiceK_aberta`/`foiceK_fechada` e `trono_aberto`/`trono_fechado`, a cada 2 s). Sem isso, as IAs de Hades e Poseidon, que atacam o edifício mais perto do inimigo mais fraco (os Altares), os derrubavam sozinhas; com um só soldado, um perseguidor perdido "abria" o Altar para o exército aliado. Hades explica na abertura: *"Um detalhe, arconte: juramos pelo Estige, e nenhuma arma nossa fere um altar do nosso pai. As de Argos, sim."* — e é por isso que os deuses precisam dos exilados (a fala antiga, *"juramos nunca erguer a mão contra um altar"*, contradizia a tela: os exércitos dos irmãos batiam nos Altares). **A IA não mira o que não pode ferir** (revisão): `chooseAttackTarget`/`alliedAttackTarget` (`src/core/sim/ai.ts`) ignoram edifício com piso 1, e uma onda em curso volta ao ponto de encontro quando o alvo ganha o piso. Antes, todas as ondas de Hades e Poseidon miravam um Altar (os edifícios do Culto mais perto dos CCs deles; no Normal, 518 s e 375 s de 1200 com um Altar como alvo) e ficavam presas batendo na pedra; agora elas atacam as torres da Estrada do Trono e a base do Culto no Ótris.
- **Guardas e vinganças da Foice.** Três sentinelas do Culto (estátuas imóveis de Hades, que a IA do Culto não leva para casa) em volta de cada Altar; cada Altar que cai manda 8 soldados do Culto (escalados por G3) da Estrada do Trono contra a Nova Argos, com a fala de Lícaon (*"Quebraram um prato da mesa do pai…"*), como as vinganças da m4.
- **As três idades ([G9]).** 1ª: piso 2/3 (preso nele enquanto houver Altar). 2ª: se o Trono estiver de pé, Cronos volta a ele (`recuo`, coleira de 14 tiles em [113,36]) e só volta a sangrar com o Trono no chão e 150 s passados — o **cerco** do objetivo-tipo (objetivo `trono`, revelado quando ele sai sem os Altares); sem Trono, ele "devora a hora" ali mesmo (150 s de piso 1). 3ª: devora a hora (150 s) e depois fica sem piso — o Raio de Zeus (metade da vida de um Titã) basta. Cada idade chama uma onda do Culto junto dele (`fome1`, `fome2`, escaladas). A ficha pedia só as ondas em 0,66/0,33; as horas de 150 s põem o mínimo do Difícil em 25 min (20 + 2 × 2m30s), acima da janela (a medição com horas de 60 s vencia no Difícil aos 24m30s, na borda).
- **Dificuldade com [G3]** (a proposta da ficha): prazo de 30 / 25 / 20 min; no Difícil, Cronos surge com 2 Colossos e recupera 10 % da vida ao começar a 2ª e a 3ª idades (como o golfo da m8).
- **Os Altares e o Trono com nome ([G8]).** Eles nascem do `setup` (`place` com `exact`, `tag` e `name`: "Altar da Foice" / "Altar of the Sickle" e "Trono de Cronos" / "Throne of Cronus"), antes do piso do juramento, como as jaulas e fendas da m9; o mapa só reserva o chão (entidade de mapa não tem nome). Antes, ao selecioná-los, o jogador via "Templo" e "Fortaleza".
- **O Trono** é uma Fortaleza do Culto com o canto em [111,29] (cobre [111..114]×[29..32]; a ficha dizia [112,30]); Cronos surge num ponto fixo ao pé dele, [113,36] (o Trono pode já ter caído). Ele é invulnerável **antes de Cronos sair** — o Culto nunca fica sem edifício antes do chefe (um Cronos de jogador eliminado não seria alvo de ninguém: `canTarget` ignora donos mortos) — e enquanto houver Altar; depois, cai como eles.
- **A Nova Argos.** A ficha lista CC, Templo, Academia, Fortaleza em obra (`complete: false`), 15 cidadãos, Perseu, Odisseu e Héracles; somam-se **4 Casas** (15 cidadãos e os heróis passavam do teto de 20 do CC), as **3 torres** das passagens e a **guarda do êxodo** (6 hoplitas e 4 toxotas no `setup`): sem elas, o 1º ataque do Culto e o devorar derrubavam a cidade parada aos 5m18s.
- **O Culto** joga como IA **normal** na `config` (Difícil → difícil), com estoque próprio (600 / 600 / 500 / 100 / 300), deuses menores Ares, Afrodite e Hera (Ciclopes, Leões de Nemeia e Medusas, as do devorar) e 4 pesquisas. Com a IA difícil da ficha (Difícil → muito difícil) e o estoque cheio, ele tinha 90 militares aos 12 min, derrubava Poseidon aos 7 min e a Nova Argos do roteiro aos 13–15 min no Difícil.
- **Os irmãos** ficaram em IA normal (a ficha), com deuses menores (Hades: Atena, Apolo e Hefesto; Poseidon: Ares, Dionísio e Hefesto — sem Hermes, cujo Cessar-fogo pararia o mapa inteiro) e 4 pesquisas. Testado: como IA difícil eles seguram as próprias casas, mas a Nova Argos parada vira o alvo "mais fraco" do Culto e cai antes (12m19s no Normal, contra 20m55s).
- **[G6]:** `maxAge: 3` em Hades, Poseidon e no Culto (o Titã de Hades é o próprio Cronos, e o de Poseidon, Oceano) e `forbid` de Perseu, Héracles e Odisseu nos três; `forbid` global de Aquiles (morto na m7) e Jasão (saiu na m8). O harness confere 1 Titã de cada.
- **devorar** em três gatilhos (`devorar`, `devorar_hades`, `devorar_poseidon`), cada um só com o CC do alvo de pé (o `forEachPlayer` da ficha gerava invasão sem alvo quando um CC caía, e a checagem (f) falhava), todos vindos do lado do Campo (ângulos 7 e 5 para os irmãos). **O da Nova Argos não é mais `raid`** (revisão): a origem do `raid` (ângulo 1, 22 tiles) caía na própria rampa, a 6,4 tiles da `torre_rampa`, e, com as três passagens muradas, a espiral achava tiles **dentro** das muralhas (a 13–15 tiles do CC). Agora o grupo nasce num ponto fixo da estrada do Olimpo, no Campo ([50,51], a ~20 tiles da torre), e ataca-move até o CC (`devorar_marcha` empurra de novo os ociosos a cada 5 s, como as vinganças); com as passagens muradas, ele fica do lado de fora (a IA do Culto o leva contra as muralhas).
- **Segredo de Perseu com [G13]** ("Perseu dá o golpe final em Cronos", `kills … by { type: "perseus" }`) no lugar de "Perseu vivo": o Raio e os golpes de outros não contam, e a fala final é de Perseu quando o golpe é dele e de Zeus quando não. Com `by { tag: "perseu" }` (1ª versão) só contava o Perseu do `setup`; o herói é único e pode ser retreinado no Templo, e o retreinado não herdava a tag — por isso o G13 ganhou `by.type` (qualquer autor desse tipo do jogador).
- **HUD ([G4]):** contagem até Cronos sair (uma por dificuldade), "🔥 Altares da Foice derrubados n/3", "⏳ Idade de Cronos n/3" e a contagem da hora devorada.
- **Falas novas** (além das da ficha): Hades (o juramento), a Pítia (os Altares só com soldados de Argos, o Portal aos 90 s, os Altares caídos, o aviso 3 min antes do prazo, o Trono exposto, a hora devorada, o Tempo que sangra), Odisseu (o plano: um Altar por vez), Lícaon (o devorar, a vingança, os Colossos do Difícil), Hades (*"Sem trono, ele é só um velho com fome"*) e Prometeu ao atender.
- **Recursos da costa:** a praia (areia) não tem árvores, e a costa de Poseidon ficou sem madeira a 16 tiles do CC (`map:check`); ganhou dois bosques e ouro no capim, sem gargalo novo.
- **Fim da campanha:** a vitória na m12 mostra o `outro` (o epílogo da ficha, em dois parágrafos) e *"🏛️ Fim da campanha: a Titanomaquia terminou."*, sem "Próxima missão" (`hud.ts` só a oferece quando `nextCampaignMission` existe; a Horda também deixou de oferecê-la). As conquistas "Ato III completo" e "Titanomaquia no Difícil" já contam a m12 (vêm de `CAMPAIGN_PLAN`), mas só destravam com m10 e m11 registradas e cumpridas. Efeito colateral: "Argonautas" (três heróis vivos) destrava no 1º segundo da m12.
- **Passiva:** derrota aos **33m01s / 12m57s / 10m00s** (Fácil / Normal / Difícil; na 1ª versão, 46m39s / 20m55s / 8m45s). No Fácil, Cronos sai aos 30 min e derruba a Nova Argos parada aos 33m01s, como a §7.2 previa ("os aliados seguram; aos 25 min Cronos surge e derruba a Nova Argos: derrota em 30–40 min"); no Normal e no Difícil quem a derruba é o Culto, antes de Cronos, e os irmãos seguram as próprias casas. A queda no Normal (20m55s → 12m57s) vem da IA corrigida: as ondas dos irmãos não ficam mais presas nos Altares, onde o exército do Culto as enfrentava (ele defende os próprios Templos); agora elas derrubam a base do Culto no Ótris (o CC dele cai aos ~9 min nos roteiros) e o exército do Culto sai contra a cidade parada, o alvo "mais fraco".
- **Checagens de fim do roteiro (revisão).** O piso da janela estrita (24m30s) é estrutural: Cronos só sai no prazo e as duas horas devoradas duram 150 s cada, então a vitória mais rápida é prazo + 5 min (35 / 30 / 25 min). Por isso o `atEnd` da m12 confere também o que o jogador controla: os Altares no chão antes de Cronos sair (`altares_caem`) e Cronos derrotado em até 15 min depois de sair (`time ≤ prazo + 900`), além de Cronos em ataque por ≥ 25 s.
...
- **Com [G13] ✅ — "Perseu dá o golpe final":** um segredo com `"done": { "kills": { "player": 0, "type": "cronus", "by": { "type": "perseus" } }, "gte": 1 }` (o Raio, que só tira metade da vida de um Titã, e os golpes de outros não contam) e uma fala própria de Perseu no lugar da de Zeus quando for ele.

**Diálogos-chave:**
- **Héracles** (quando Argos chega ao 1º Altar, se ele estiver vivo): *"Uma foice gigante? Já enfrentei coisa pior. Quer dizer... não. Mas vamos lá."* / *"A giant sickle? I've faced worse. Well... no. But let's go."*
...
- **Perseu** (Cronos surge, se ele estiver vivo; o retreinado também fala): *"Minha espada foi feita para isto. Abram caminho."* / *"My sword was made for this. Clear the way."*
- **Zeus** (Cronos cai): *"Pai. Você engoliu uma pedra no meu lugar. Hoje o mundo inteiro te cospe de volta ao Tártaro."* / *"Father. You swallowed a stone in my place. Today the whole world spits you back into Tartarus."*
- **Revisão da mitologia (26/09/2026; o texto é da ficha: confirmar com o dono).** Cronos engoliu Héstia, Deméter, Hera, Hades e Poseidon; Zeus foi salvo por Reia, que deu a Cronos uma pedra enrolada em panos. As duas falas de Zeus diziam o contrário (*"Da última vez que estivemos juntos, estávamos dentro da barriga dele"* e *"Você me engoliu uma vez"*) e foram trocadas pelas de cima (a abertura: *"Irmãos. Da última vez que vocês estiveram juntos, eu os tirei da barriga dele."*; a resposta de Hades, *"Não se acostume."*, continua valendo).

**Outro (epílogo da campanha):**
- PT: *"Cronos voltou ao Tártaro, acorrentado pelos Ciclopes com bronze novo. No Olimpo, Zeus ofereceu ao arconte um lugar entre as estrelas. O arconte recusou."* / *"O arconte voltou às cinzas de Argos e, com a brasa que um velho sacerdote salvou do fogo, acendeu a nova lareira comum. Os deuses venceram. Os homens sobreviveram. É tudo o que uma tragédia concede."* (dois parágrafos no jogo; o 2º nomeia o arconte, que não tem gênero, §1.2)
- EN: *"Cronus has returned to Tartarus, chained by the Cyclopes in new bronze. On Olympus, Zeus offered the archon a place among the stars. The archon refused."* / *"The archon went back to the ashes of Argos and, with the ember an old priest saved from the fire, lit the new common hearth. The gods have won. Mankind has survived. That is all a tragedy grants."*
...
| **G10** ✅ | Relíquias em cenário | **Feito.** `map.relics` (cenário em mapa gerado → `config.relics`) e `FixedMapData.relics`: `boolean \| [x, y][]` — lista = posições fixas (tiles, na ordem da lista, até `MAX_FIXED_RELICS` = 32; `[]` = nenhuma), por `placeRelicsAt` (`src/core/sim/relics.ts`, sem `state.rng`; tile que não é terra livre na região dos inícios vai à terra alcançável mais próxima até `RELIC_SNAP_RADIUS` = 6, senão sai). `validateMap`: `relicsFormat`, `relicOut`, `relicDup`, `relicBlocked`, `relicUnreachable`, `relicsCount` (erros, PT/EN); com mapa fixo valem só as relíquias dele (`config.relics` de um cenário embutido é ignorado); `validateScenario`: formato, limites do tamanho, repetidas e `map.relics` junto de `data` (use `data.relics`); lint: gera o mapa pela semente e avisa a posição que não é terra alcançável. `mapHash`/`canonicalize`/redimensionar (`ResizeReport.relics`) e o editor (sobreposição das fixas, Corrigir mover/remover desfazível, borracha e 🏺 Pôr relíquia nas Propriedades) cobrem a lista | `Value { "stat": "relics", "player" }` = `relicsOf` (guardadas agora nos Templos) | — | m9 (secundário) | **feita** |
| **G11** ✅ | Poderes específicos | **Feito.** `{ "do": "set", "player", "powers": { "remove", "add", "reset" } }` (nesta ordem; ids de `POWERS`): remove tira (até o do deus maior), add concede sem repetir, reset devolve o uso a quem já tem. Barra de poderes, controle, IA e comando `power` leem a mesma lista (`player.powers`); tirar o poder mirado cancela a mira. `{ "powerUsed": { "player", "id" } }` (sem comparação = ao menos 1; com `Cmp`, usos desde o início): `state.scenario.powerUses`, gravado por `recordPowerUse` em `usePower` (`src/core/scenario/log.ts`), salvo no save (padrão `{}`); reset/remove não zeram. O evento `powerUsed` traz o id (o som de um poder reposto toca de novo) | m7 **não mudou** (Tétis continua; como ficaria sem o Raio: §5.4, decisão do dono) | — | m11 (2º Raio), m7 (tirar o Raio contra Aquiles), variações | **feita** |
| **G12** ✅ | Habilidade de herói por roteiro | **Feito.** `{ "do": "ability", "unit": EntityRef, "target"?: EntityRef \| ponto }`: `useAbility` (o caminho do comando `ability`: recarga; guarnecido, morto ou sem habilidade não usa; vale para marionete). Usada a habilidade, `target` de outro time = atacar, do mesmo time = ir até ele, ponto (`at`/`start`/`entity`/`tc` com `dx`/`dy`) = atacar-mover (`isPointLike`) | m7 **não mudou** (a Fúria no Difícil mudaria os tempos; exemplo em §5.4) | — | m7 (Fúria no Difícil) | **feita** |
| **G13** ✅ | Autoria de abate | **Feito.** `killUnit`/`destroyBuilding` chamam `recordKill` (`src/core/scenario/log.ts`) em toda morte com autor inimigo: `state.scenario.kills.byPlayer` (`"jogador:tipo da vítima"`) e, com entidade autora (golpe, flecha, dano em área, petrificação; `destroyBuilding` ganhou o autor opcional), `byEntity` (`"jogador:id"` → tipo → n). Só soma contadores (sem `state.rng`, entidades ou eventos): `scripts/missions.ts` dá os mesmos tempos. Salvo no save (padrão vazio; malformado é descartado) | `{ "kills": { "player", "type"?, "by"?: { "tag" } \| { "type" } }, "gte": 1 }`: `type` = tipo(s) da vítima (unidade ou edifício), `by.tag` = grupo atual da tag (autor morto conta), `by.type` = qualquer autor desse tipo do jogador (o herói único retreinado conta; registro `byType`). Poderes e atrito contam para o jogador, não para a tag; `kill`/`damage` do roteiro, dispensar e alicerce (obra incompleta) derrubado não contam (como `stats.razed`). Lint: `by.tag` futura como as demais | — | m12 ("Perseu dá o golpe final") | **feita** |
...
| **G17** ✅ | Contador de `repeat` | **Feito.** Todo gatilho `repeat` (JSON e TS) conta os disparos em `vars['@id']`, já somado quando o `then` roda (1 no 1º); `"repeat": { "max": N }` (`TriggerDef.maxFires`) para no teto e `setVar '@id' 0` rearma. Repeat continua fora de `fired` | Lint: `{ "fired": <repeat> }` (use `{ "var": "@id", "gte": 1 }`) e `@x` sem gatilho repeat `x`; `hud.progress { "var": "@id" }` não pede `setVar` | — | ondas fixas ficam mais curtas | **feita** |
...
**Missões que não dependem de nenhuma lacuna** (vão primeiro para a produção): **m4, m5, m6, m7, m8 e m9** (a m9 pode usar [G10] ✅ para as relíquias). Todas usam só a gramática atual, com gatilho espelho para segredos e contadores por segundo. As três seguintes dependiam de lacunas que agora estão **feitas** e podem usar a gramática direto, sem paliativo:
- **m10:** G2 ✅ + G4 ✅;
- **m11:** G8 ✅ + G9 ✅ + G6 ✅;
- **m12:** G6 ✅ + G9 ✅ (+ G13 ✅ para o golpe final de Perseu).
...
| 4 | **m9** ✅ (mapa gerado; produzida) + **m10** | **G8** nomes ✅; **G6** remove/maxAge ✅ | Ato III pela metade |
| 5 | **m11** ✅ (mapa fixo "Argólida em Chamas"; produzida) + **m12** | **G9** vida de chefe ✅ | Campanha completa |
| 5 | **m11** + **m12** ✅ (mapa fixo; produzida) | **G9** vida de chefe ✅ | Campanha completa |
...
| m8 | Oceano derruba o CC: **derrota** em 12–20 min. **Medido (revisão, `npx tsx scripts/missions.ts 22 m8_oceano`):** derrota aos 18m02s / 16m09s / 13m14s (Fácil / Normal / Difícil). Quem derruba o CC da Argos parada é a Liga, durante o 1º recuo, nas três (Oceano está na praia; na 1ª maré ele esmaga o exército parado na brecha, e no Normal o Raio de Micenas o manda de volta cedo). A medição da produção (15m47s / 18m53s / 12m56s) dava "Oceano na 2ª maré" no Normal, mas ele estava parado na praia: quem derrubou o CC foi a Liga. Com a passiva padrão de 14 min, em jogo no Fácil e no Normal | IA econômica, 8 torres no sul, Raio em Oceano ao surgir, heróis em `attack` na tag `oceano`. **Produção:** defesa (`hold`), cidadãos até 45, filas militares com os três heróis (Aquiles morreu na m7), a brecha fechada com muralha, 8 torres atrás das passagens, a Hidra de Lerna com dois heróis aos 4 min, Jasão guardado atrás do CC, heróis em Oceano na maré alta e todos para dentro da muralha na baixa, Raio na 3ª maré (detalhes abaixo); o chefe precisa ter lutado (`atEnd`); variante "titãs" | 30–35 min (§4) · janela estrita 21m–45m30s (o "13–25" desta linha era a estimativa do rascunho, com uma luta só). **Medido (revisão, com o Titã lutando):** vitória aos 25m22s / 24m15s / 24m56s; marés aos 10 min, ~17 min e ~24 min; `lerna` e `jasao` cumpridos nas três; Oceano em `attack` por 41–135 s por partida (antes da correção da coleira, 1–15 s). **Variante "titãs"** (Prometeu pelo Portal; exige `prometeu`): 24m45s / 24m36s / 24m56s, com `titas` cumprido nas três e Prometeu por volta dos 11 min. Antes da revisão: 23m56s / 23m41s / 23m45s (variante 25m02s / 23m45s / 23m51s), com o chefe sem lutar. |
| m9 | Ondas das fendas até o Palácio cair. **Medido no rascunho:** derrota aos 15, 10 e 8 min (Fácil, Normal, Difícil). **Medido (produção):** derrota aos 13m39s / 11m25s / 7m36s; 12 / 11 / 9 invasões até a derrota, todas com unidades. **Depois da revisão** (ondas nascendo nas fendas, sem `raid`): 13m04s / 11m42s / 8m50s | Míticas e heróis fazem `attackMove` para `jaula1..3` e depois `fenda1..3`. **Produção:** casas antes do teto de população, filas com as míticas do Templo, e um alvo por vez na ordem da caminhada (`fenda1` → `jaula1` → `jaula2` → `jaula3` → `fenda3` → `fenda2`), cada um quando o exército tem ≥ 40 militares (detalhes abaixo) | 22–35 min (§7.2) · janela estrita 17m30s–39m (§4 ±30 %). **Medido:** vitória aos 24m53s / 22m42s / 33m52s (Fácil / Normal / Difícil); `miticas`, `cerbero` e `elmo` cumpridos nas três, segredo `sombras` em nenhuma; 20 / 22 / 50 invasões, todas com unidades; o Culto é eliminado entre os 21 e os 26 min. **Depois da revisão:** vitória aos **21m34s / 22m51s / 32m00s**; `miticas` e `cerbero` nas três, `elmo` só no Normal, `sombras` em nenhuma; o Culto cai aos ~21 min no Fácil e no Normal e só no fim no Difícil |
| m10 | Sortidas até o CC cair: **derrota** em 15–25 min. **Medido (depois da revisão, `npx tsx scripts/missions.ts 26 m10_otris`):** em jogo aos 26 min no Fácil (4 sortidas de 4) e derrota aos 18m57s / 14m32s no Normal / Difícil (3 sortidas de 6 / 2 de 9); a mesma cadência nas três, o tamanho escala; o Culto, trancado, não sai; com a passiva padrão de 14 min, em jogo nas três | Exército no altar até `altar_s` = 120; helépoles em `portao_bronze` e depois nos pilares. **Produção:** a IA do jogador segura o altar sozinha (o Rei da Colina reúne o exército lá; `altar` cumprido aos ~2m30s) e enfrenta as retomadas dele (`atEnd`: ao menos uma sortida); os passos põem as máquinas de cerco (4 helépoles e 2 petróbolos, com cofre), o Mercado quando falta ouro e o cerco por estado (detalhes abaixo) | 25–40 min · janela estrita 21m–45m30s · **medido: 24m36s / 40m19s / 40m25s** |
| m11 | Ninguém embarca: **derrota** aos 18 min (ou antes, se o arconte cair). **Medido (produção, `npx tsx scripts/missions.ts 14 m11_chamas`):** o arconte, parado junto ao CC, cai no 1º ataque do Culto: derrota aos **6m51s / 9m14s / 8m06s** (Fácil / Normal / Difícil; o Culto "fácil" ataca antes, com menos soldados); as ondas da passiva (2, até a derrota) têm unidades | Arconte e cidadãos fazem `move` até `naus` em 3 levas (aos 200, 500 e 740 s); poderes usados nos grupos das ondas. **Produção:** levas uma por nau, 70 s antes dela, com os lugares + 2 (a última leva leva todos e sai 80 s antes de Cronos: 170 / 470 / 640 s no Normal, 670 s no Fácil, 520 s no Difícil); o arconte e o exército inicial descem com a 1ª leva, o arconte fica atrás do cais em postura passiva e a escolta guarda a boca do porto; na última leva, o exército da cidade também desce; cada poder na onda dele (detalhes abaixo) |  18 min (§4, fixo) · janela estrita **12m36s–23m24s** (a vitória só pode sair antes dos 18). **Medido (revisão):** vitória aos **13m11s / 13m19s / 14m19s** (Fácil / Normal / Difícil) — o relógio das naus (3ª aos 13 min; 4ª aos 14 no Difícil) mais o embarque; Cronos desce antes do fim nas três (750 / 720 / 600 s; no Fácil ele chegava aos 840 s, depois da vitória); os quatro objetivos cumpridos nas três (`retaguarda` e o segredo `chama`, aos 240 s, quando a 1ª nau atraca); Maldição na 1ª onda (121 s) e Tempestade nos arqueiros que caçam o arconte (331 s) nas três, Trégua na cavalaria do cais no Fácil e no Normal (573–574 s); **o Raio e a Restauração não são usados** (o 2º Raio, `raio2`, não dispara no roteiro: só o teste unitário o cobre); 3 invasões, todas com unidades. **O arconte não é posto à prova:** estacionado atrás do cais (passivo) com a escolta na boca do porto desde os 170 s, fica com 100 % da vida até o fim nas três (`arconte_ferido` nunca dispara) — a estratégia anula a caça das ondas (`ira2`, `hostes`); a pressão real sobre o líder fica para o playtest humano calibrar |
| m12 | Os aliados seguram; aos 25 min Cronos surge e derruba a Nova Argos: **derrota** em 30–40 min. **Medido (produção):** em jogo aos 14 min no Fácil e no Normal, derrota aos 8m45s no Difícil; passiva longa: derrota aos 46m39s (Fácil, depois de Cronos aos 30 min) / 20m55s / 8m45s — no Normal e no Difícil quem derruba a Nova Argos parada é o Culto, antes de Cronos (os irmãos seguram as próprias casas) | IA econômica com os aliados; o exército faz `attackMove` nos 3 altares; Raio e Perseu em Cronos. **Produção:** defesa (`hold`), Raio guardado (`keepPowers`), cidadãos até 45, Casas antes do teto, a Fortaleza terminada por 4 cidadãos, Quartel e Oficina, filas com `M12_MIX`, Restauração nos feridos, os Altares na ordem da caminhada com ≥ 50 militares, Cronos com heróis nele quando pode ser ferido, o cerco ao Trono no recuo, Raio na 3ª idade; `atEnd`: Cronos em ataque por ≥ 25 s, os Altares antes de Cronos sair e Cronos derrotado até 15 min depois de sair (o piso da janela é estrutural: prazo + 5 min) (detalhes abaixo) | 35–40 min (§4) · janela estrita 24m30s–52m (a §7.2 dizia 30–45). **Medido:** vitória aos **35m54s / 31m24s / 27m42s** (Fácil / Normal / Difícil); Altares aos 6m35s–7m24s / 6m34s–7m39s / 6m38s–9m39s; Cronos aos 30 / 25 / 20 min; 2ª idade no Trono no Fácil e no Normal (ele cai aos ~33 e ~28 min) e na hora devorada no Difícil (o Trono caiu logo que Cronos saiu); Cronos em ataque por 294 / 296 / 452 s; `irmaos` cumprido nas três, `prometeu` no Fácil e no Normal (a IA do jogador sobe à Idade dos Titãs sozinha), segredo de Perseu em nenhuma; 6 / 6 / 9 invasões, todas com unidades. **Depois da revisão** (IA que não mira Altar sob o juramento, devorar da Nova Argos nascendo no Campo, Altares e Trono com nome): passiva em jogo aos 14 min no Fácil e derrota aos **12m57s** (Normal) e **10m00s** (Difícil); passiva longa **33m01s / 12m57s / 10m00s**. Roteiro: vitória aos **35m58s / 30m34s / 27m49s**; Altares aos 6m38s–7m25s / 6m25s–7m11s / 6m35s–7m41s (Héracles fala aos ~6m10s–6m22s, quando o exército chega ao 1º); 2ª idade no Trono **nas três** (ele cai aos 31m27s / 25m56s / 24m41s); Cronos em ataque por 312 / 286 / 245 s; ondas dos irmãos com alvo sob piso 1: 0 / 0 / 4 s (antes, 518 s e 375 s de 1200 só na passiva do Normal); `irmaos` nas três, `prometeu` no Fácil e no Normal, segredo em nenhuma; 12 / 10 / 8 invasões na passiva de 14 min e 4 no roteiro (só os `raid` de Hades e Poseidon), todas com unidades; `atEnd` sem pendências |
...
- **Variantes** (`MissionScript.variants`, `runMissionScript(id, d, det, rótulo)`): outros caminhos medidos da mesma missão, cada um com a própria janela e com gatilhos exigidos (`fired`) e proibidos (`notFired`), conferidos por `variantIssues`; `scripts/missions.ts` roda todas logo depois do roteiro, com o mesmo rigor. Hoje a m5 tem uma ("escolta") e a m8 outra ("titãs").
- **Estrito por padrão** (`scriptVerdict`): `npx tsx scripts/missions.ts [min=14] [ids] [dificuldades]` falha se o roteiro não vencer dentro de `expect` em qualquer dificuldade. Só `exceptions: { hard: '<motivo>' }` (motivo obrigatório, usado depois de esforço honesto) dispensa uma dificuldade — e a exceção é listada no fim da saída ("Missões: OK (com N exceção(ões) declarada(s))"). Hoje não há nenhuma: m1 [10m30s–26m], m2 [14m–32m30s], m3 [17m30s–39m], m4 [15m–39m] (revista em 26/09/2026, ver o roteiro da m4), m5 [14m–32m30s], m6 [17m30s–39m], m7 [14m–39m] e m9 [17m30s–39m], m8 [21m–45m30s] e m12 [24m30s–52m] e m11 [12m36s–23m24s] e m10 [21m–45m30s] vencem nas três dificuldades. Referência de 26/09/2026 (depois da correção do viés de posição, Fácil / Normal / Difícil): m1 14m39s / 14m26s / 15m55s, m2 19m33s / 15m32s / 15m25s, m3 18m30s / 19m10s / 19m16s, m4 16m14s / 19m00s / 19m09s, m5 15m55s / 16m55s / 17m55s (variante "escolta" 6m08s / 7m55s / 8m59s), m6 19m36s / 20m02s / 22m17s, m7 17m47s / 17m00s / 22m52s; m8 (produzida depois, na mesma base; revisada com o Titã lutando) 25m22s / 24m15s / 24m56s (variante "titãs" 24m45s / 24m36s / 24m56s); m9 (produzida depois da correção) 24m53s / 22m42s / 33m52s; m10 (produzida depois; revisada com o altar disputado) 24m36s / 40m19s / 40m25s; m11 (produzida depois, na mesma base; revisada: segredo só com lugar a bordo, Cronos antes da última nau no Fácil, a cavalaria que não trava nas naus) 13m11s / 13m19s / 14m19s; m12 (produzida por último) 35m54s / 31m24s / 27m42s. O roteiro é uma partida de IA, caótica: qualquer mudança na IA ou no motor muda esses tempos. Uma quebra se diagnostica comparando as duas versões com uma sonda (militares, população, casas, ondas e objetivos a cada 30 s). Só depois se decide entre corrigir a IA (defeito real) e ajustar o roteiro (um jogador faria diferente). Nunca se freia o roteiro para caber na janela.
...
- Roteiro da **m7** (`MISSION_SCRIPTS.m7_aquiles`, janela 14m–39m, `hold` sempre): a IA do jogador guarda o Oráculo e o Raio (**poderes guardados**, `keepPowers`, utilitário novo do harness: durante o `aiThink` a IA os vê como já usados; os passos os usam na hora certa; sem o campo, nada muda) — ela gastaria o Oráculo no 2º segundo e o Raio no primeiro mirmidão perto de casa. Oráculo quando Aquiles marcha (`rota1`), **vigia** (`m7Guard`: depois da 1ª marcha, o exército espera Aquiles na aldeia da vez — a da marcha em curso, se ainda de pé com inimigos perto, ou a próxima ainda não atacada — quando tem ≥ `M7_GUARD_RATIO` = 2 × quem marcha com ele: Aquiles, os mirmidões do desembarque vivos e 5 de escolta; com inimigos a até 22 tiles do CC, volta para casa. Medido na revisão: sem essa folga, 18 militares na aldeia2 aos 5 min morriam todos e a IA do jogador não se reerguia, derrota aos 25–29 min no Normal e no Difícil), Raio/Restauração por `battlePowers`, Estábulo e Oficina de Cerco se a IA não os ergueu (sem petróbolos, as flechas mal arranham a Fortaleza e os assaltos do Difícil morriam nela até os 32–34 min). **Assalto às naus por estado, não por relógio:** com ≥ 80 militares e ≥ 2 × os do jogador 2 (continua enquanto metade do exército estiver a até 18 tiles delas: alguns perseguidores que chegam às naus atrás dos mirmidões não arrastam o exército inteiro); antes de avançar, sem ninguém lutando nas naus, o exército se **reagrupa** a `M7_STAGE` = 26 tiles delas (no rumo de Argos) e só avança com ≥ 70 % no ponto; perto delas, infantaria, cavalaria e cerco batem no acampamento e os arqueiros ficam nos mirmidões. Queimadas as naus, todos atrás de Aquiles. Medido nos rascunhos: com 55–70 militares a vitória saía aos 13–15 min em alguma dificuldade (no limite ou abaixo da janela); sem reforços fixos das naus, o Fácil ficava no limite (14m02s); com a pilha de mirmidões parando nas cinzas, a IA do jogador perdia o CC aos 5–7 min ou vencia aos 5–10 min por acaso.
  - **26/09/2026** (correção do viés de posição): o Fácil vencia aos 13m58s, abaixo da janela. A causa não era o roteiro, e sim a IA da Liga. A nova ordem de `findBuildSpot` encostava a mina no único veio de ouro dela (em [103,43], com 2 tiles de acesso) e o fechava. O nó sumia para `nearestNode`, e os mineiros iam ao ouro junto de Argos, a ~60 tiles. Ela perdeu 19 cidadãos até os 4 min e desmoronou. Corrigido na IA: `sealsNode` impede que um edifício tire o último acesso de um recurso. Depois disso, o 1º assalto do Normal e do Difícil chegava às naus aos poucos: 14 militares aos 14m00s e o resto 10–20 s depois, e o exército caiu de 82 para 39 em 1 min. Normal e Difícil passaram a vencer só aos 29m08s / 29m16s. Com o reagrupamento, o resultado é 17m47s / 17m00s / 22m52s. `aldeias` agora falha também no Fácil: a aldeia3 cai. Medido pelo revisor: sem o reagrupamento (e já com `sealsNode`) a m7 dá 15m13s (`aldeias` cumprido) / 29m08s / 29m16s — dentro da janela, então o reagrupamento não foi posto para caber nela; ele encurta Normal/Difícil, mas custa 2m34s e a aldeia3 no Fácil. Detalhe: com mais de 30 % do exército lutando longe do ponto, o passo espera o combate acabar.
- Roteiro da **m8** (`MISSION_SCRIPTS.m8_oceano`, janela 21m–45m30s, `hold` sempre, Raio guardado com `keepPowers`): cidadãos até 45; filas militares com os três heróis de Argos (dano triplo em míticas, e o Titã é uma; Aquiles morreu na m7 e está proibido), hipaspistas, mirmidões, arqueiros cretenses, Minotauros, hetairoi e petróbolos; a brecha da crista fechada com muralha (a dica de Jasão) e 8 torres atrás das três passagens; aos 4 min, com dois heróis, a Hidra de Lerna (`lerna`); pouco antes da 1ª maré, **Jasão sai da linha** (postura passiva, atrás do CC, destacado da IA do jogador: 200 de vida são dois golpes do Titã; é o objetivo `jasao`). **Na maré alta**, com Oceano a até 34 tiles do CC, heróis (menos Jasão) e Prometeu batem nele e o resto do exército vai em ataque-movimento até ele; **na baixa**, quem ainda o persegue desengaja com `move` e todos voltam para dentro da muralha. Medido: com ataque-movimento, o Titã a um passo era o alvo de novo, e 76 de 82 militares morreram atrás dele na praia no 1º recuo. Na 3ª maré, o Raio nele (`m8Bolt`). A vitória depende pouco da força do exército: o relógio das marés (10 min + 6 + 6, mais as lutas) põe o mínimo perto dos 23 min. Por isso a janela não prova que o chefe lutou, e o roteiro (e a variante) declara `atEnd` (novo no harness, `ScriptEndCheck`: condição no estado final ou por N segundos da partida; `scriptVerdict` falha com o rótulo da que não valer): **Oceano em `attack` por ao menos 25 s**. Com a coleira antiga, que o punha em `move` desde a subida, o roteiro vencia dentro da janela com 1–15 s de ataque; corrigida, 41–135 s. Abates não serviriam de prova: com o dano em área, 8 s de ataque na brecha já davam 40.
  - **Variante "titãs"** (a 3ª resposta da ficha; exige o gatilho `prometeu`): as pesquisas que faltam na Academia, a Fortaleza, a Idade dos Titãs e o Portal (o roteiro põe os construtores: em cenário, a IA não os põe no Portal), com **dez cidadãos rezando** no Templo (destacados da IA) e **cofres em lista** (`reserve` agora aceita uma lista, somando os que valem; com um só, nada muda): Favor e Conhecimento até Prometeu, madeira e ouro da Fortaleza até ela sair, comida e ouro da Idade até ela e o custo do Portal até ele. Sem Favor guardado e sem quem rezasse, a Idade e o Portal só saíam depois dos 20 min (e o Difícil perdia a cidade aos 20m16s, com o exército gasto na 1ª maré sem heróis); a variante também não enfrenta a 1ª maré sem ao menos dois heróis ou Prometeu (`cautious`). Prometeu atende por volta dos 11 min e bate em Oceano junto dos heróis.
- Roteiro da **m9** (`MISSION_SCRIPTS.m9_tenaro`, janela 17m30s–39m, `hold` sempre): a IA do jogador joga por Hades (economia, defesa do Palácio, Sentinelas) e nunca sai em ondas; os passos treinam cidadãos até 40, erguem uma **Casa** perto do Palácio sempre que faltam menos de 15 de população (`m9House`: a IA só ergue a próxima com ≤ 6 de folga e as míticas ocupam 3 a 5 cada; sem isso o Normal ficava em 70/70 dos 7 aos 14 min e só fechava a 1ª fenda aos ~19 min, com 2 jaulas abertas aos 40 min), enchem as filas com `M9_MIX` (Minotauros, Mantícoras e Cérberos do Templo, hipaspistas, arqueiros cretenses e petróbolos) e atacam **um alvo por vez, na ordem da caminhada** a partir do Palácio (`M9_TARGETS`: `fenda1` a 38 tiles a pé, `jaula1` 64, `jaula2` 84, `jaula3` 87, `fenda3` 101 e `fenda2` 160), cada um quando o exército tem ≥ `M9_ASSAULT_ARMY` = 40 militares (estado, não relógio; o assalto segue enquanto ≥ 10 estiverem a 16 tiles do alvo) e com foco na jaula ou na fenda a 10 tiles. A ficha pedia as jaulas antes das fendas; a caminhada põe a `fenda1` (fora do platô) primeiro e a `fenda2` (além do lago do sul) por último. Os heróis vão com o exército; a IA do jogador manda o herói ocioso buscar a relíquia mais perto (até 45 tiles), e o Raio do Culto derruba Héracles no 1º minuto nas três dificuldades (Perseu chega depois e guarda as relíquias). **Medido (1ª produção):** 24m53s / 22m42s / 33m52s. **Medido depois da revisão** (Culto sem heróis e com Centauros/Hidras, ondas nascendo nas fendas): **21m34s / 22m51s / 32m00s**; a `fenda1` cai aos 9m19s / 11m12s / 19m06s, as três jaulas aos 20m06s / 20m34s / 29m45s e a `fenda2` (a última) fecha a missão. Agora o Fácil vence antes do Normal. No Difícil Perseu cai aos ~7 min, a 1ª fenda só cai aos 19 min e a economia cai a 11–15 cidadãos depois dos 25 min; ele vence com a força reunida e os 6 Ciclopes.
- Roteiro da **m10** (`MISSION_SCRIPTS.m10_otris`, janela 21m–45m30s, `hold` sempre): no Rei da Colina a IA do jogador reúne o exército no Altar do Tempo e o segura sozinha (cumpre o `altar` aos ~2m30s, antes da 1ª sortida; depois, as retomadas das Sentinelas descem contra ela lá, a cada 4 min 30 s desde os 9, e o `atEnd` exige ao menos uma sortida enfrentada). Os passos: cidadãos até 45, Casas antes do teto de população (`m9House`), filas com `M10_MIX` (hipaspistas, mirmidões, arqueiros cretenses e as míticas dos três deuses menores: Minotauro, Mantícora e Medusa) só até 70 militares de linha (`M10_LINE_CAP`), e as **máquinas à parte** (`m10Engines`: 4 helépoles e 2 petróbolos na Oficina, antes do exército de linha), com um **cofre** enquanto houver menos de 4 máquinas (o preço de uma helépole, 250 de madeira e 200 de ouro: a IA do jogador não o gasta, e o treino de linha também o guarda) — com os Pilares cedendo só perto das máquinas, sem o cofre o ouro ficava abaixo de 100 e o Difícil passava até 10 min com 0–2 máquinas (em jogo aos 50 min numa das medições); o **Mercado** quando o ouro cai abaixo de 300 (vende o maior estoque acima de 700; sem ele, o Difícil ficava com 25 mil de comida e de madeira e 30 de ouro, sem treinar); poderes (`battlePowers`) e fuga da Tempestade. O **cerco** (`m10AssaultTarget`/`m10Assault`, a cada 5 s): um alvo por vez — o Portão de Bronze, o portão interno e o Pilar mais perto do exército —, começando com ≥ 60 militares dos quais ≥ 3 máquinas (a pedra só cede perto delas), ou com o exército cheio (≥ 75, perto do limite de população: sem isso ele parava em 84 militares sem nenhuma helépole e ficava em jogo até os 55 min no Fácil e no Difícil), e continuando enquanto ≥ 12 militares estiverem a até 18 tiles do alvo; as máquinas batem direto no alvo e o resto vai em ataque-movimento, e perto dele (10 tiles), sem militares inimigos a 8, todos batem nele. Fora do cerco, quem ficou na encosta volta ao altar (`m10Regroup`). Medido: o 1º cerco (~12 min) abre o Portão de Bronze e em geral o anel interno; os Pilares, as colmeias e o reforço do santuário custam mais uma ou duas levas — 24m36s / 40m19s / 40m25s, com 4 / 7 / 7 retomadas do altar e nenhuma perda dele. Sondagem de caos (a mesma missão com as retomadas a cada 6 min): 23m39s / 33m29s / 41m42s, todas na janela. O roteiro é caótico como os outros (§7.2, "Estrito por padrão"): as medições de calibração com a mesma missão e pequenas mudanças de números iam de 15 a 55 min, e o que deu a forma final foi tirar o que era bimodal (o 1º cerco vencer ou não) — as colmeias com piso de vida, o reforço com teto e o Culto trancado.
- Roteiro da **m11** (`MISSION_SCRIPTS.m11_chamas`, janela 12m36s–23m24s, `hold` sempre; os cinco poderes guardados com `keepPowers`; `detach` do arconte, do sacerdote, de Prometeu, da escolta e de todo cidadão do êxodo — a IA do jogador os levaria de volta à cidade): a IA do jogador cuida da economia e da defesa de Argos; os passos treinam cidadãos até a meta + 8 (contando os a bordo) enquanto a cidade existe, e fazem as **levas** (`m11Levas`: uma por nau, 70 s antes dela, com os lugares + 2, os mais perto do CC; a última leva — com todos, e depois quem nascer ou sair de uma guarnição sem inimigo a 12 tiles — sai 80 s antes de Cronos). Quem está no êxodo e parou longe do cais volta a andar até ele (a cada 3 s; cobre também o cidadão que o motor devolve à coleta no caminho, ver a pendência da §5.8). Com a 1ª leva descem o **arconte** (postura passiva, atrás do cais junto à baía) e o **exército inicial** (a escolta, na boca do porto: ataca o inimigo a ≤ 20 tiles do posto, do cais ou do arconte e volta ao posto sem ninguém); na última leva, o exército da cidade vira escolta também. **Poderes** (`m11Powers`, um por segundo no máximo): Maldição em ≥ 4 soldados humanos num raio de 4 a ≤ 14 tiles de algo que o jogador protege (edifício, cidadão ou o arconte — o batedor não conta: na 1ª versão a Tempestade caiu na base do Culto porque ele passava lá); Tempestade de Raios em ≥ 4 arqueiros a ≤ 22 tiles do arconte ou do cais (a escolta os mataria antes se esperasse) ou, depois dos 7 min, em ≥ 6 militares perto do que se protege; Trégua com ≥ 3 cavaleiros a ≤ 18 tiles do cais com gente na fila, ou ≥ 3 militares a ≤ 10 do arconte; Restauração em ≥ 6 feridos; Raio no inimigo mais valioso perto do arconte ou do cais (ou em Cronos perto de cidadãos). O roteiro declara `atEnd`: **Maldição e Tempestade de Raios usadas** e **Cronos desceu antes do fim** — a janela não prova a mecânica: pelo calendário, toda vitória possível cai entre ~13 min (a última nau + o embarque; ~14 no Difícil) e 18 min (as naus zarpam), dentro de 12m36s–23m24s; a janela só confere que houve vitória (exceção de fato à regra da §4, registrada no `MissionScript`), e a prova são as checagens do fim. A Trégua fica de fora da checagem de propósito: no Difícil a escolta (com o exército da cidade) mata a cavalaria da 3ª onda antes que ela chegue à fila, com os lugares da 2ª nau cheios, e gastá-la ali seria forçar o roteiro.
  - **Medições que mudaram a missão (só parâmetros dela):** com os lenhadores subindo até a boca do vale do Culto (14 cidadãos aos 5,5 min, 10 mortos), vieram os olivais de Argos no mapa; com a 4ª nau aos 15 min, o Culto "muito difícil" (~70 militares depois de arrasar Argos) chegava ao cais aos ~14m50s e o arconte caía aos 15m21s — a 4ª nau foi para os 14 min e Cronos deixou de seguir de Tirinto para o vau do sul (ficava a 12 tiles da boca do porto); a ordem de Prometeu contra Cronos, no mesmo disparo que invoca o Titã, era descartada (a marionete ainda não conta como viva) e foi para 1 s depois. A vanguarda de Lícaon e as pesquisas militares do Culto entraram porque só as três ondas da ficha batiam na cidade antes de Cronos; na IA do roteiro o efeito é pequeno (ela ainda chega a ~90 militares aos 8 min e esmaga a vanguarda na porta norte) — a pressão é para quem joga, que divide a atenção entre a porta norte, as levas e o cais.
- Roteiro da **m12** (`MISSION_SCRIPTS.m12_titanomaquia`, janela 24m30s–52m, `hold` sempre, Raio guardado com `keepPowers`): cidadãos até 45, uma Casa antes do teto de população (`m9House`), 4 cidadãos terminam a Fortaleza da Nova Argos (obra do mapa, sem custo; ela abre a Idade dos Titãs e a IA do jogador sobe sozinha e chama Prometeu no Fácil e no Normal), Quartel e Oficina de Cerco se faltarem, filas com `M12_MIX` (os três heróis, retreinados se caírem; Minotauros, Mantícoras e Medusas dos deuses menores; hipaspistas, mirmidões, arqueiros cretenses e petróbolos) e Restauração nos feridos (`battlePowers` gastaria o Raio na primeira mítica). **Os Altares um por vez, na ordem da caminhada** a partir da rampa do Olimpo (`foice1` → `foice2` → `foice3`), cada um quando o exército tem ≥ `M12_ASSAULT_ARMY` = 50 militares (estado, não relógio; o assalto segue com ≥ 10 a 16 tiles), com foco no Altar a 10 tiles; fora dos assaltos, quem se afastou mais de 30 tiles volta ao ponto de reunião junto da rampa. **Cronos** (`m12Fight`): quando ele marcha sobre a Nova Argos (40 tiles) ou o exército tem ≥ 50, os heróis (e Prometeu) batem nele enquanto ele pode ser ferido e o resto luta em volta dele (na hora devorada, todos contra a onda do Culto); no **recuo** da 2ª idade, o cerco ao Trono (`m12Siege`: ataque-movimento pela Estrada do Trono e, a 14 tiles, infantaria, cerco e heróis corpo a corpo no Trono); na 3ª idade sem piso, o Raio (`m12Bolt`). `atEnd`: Cronos em `attack` por ≥ 25 s (como Oceano na m8; medido: 294 / 296 / 452 s). Medido: **35m54s / 31m24s / 27m42s**. Com a 1ª versão da missão (Cronos solto pelos Altares) o mesmo roteiro vencia aos 11–16 min (Fácil e Normal) e perdia no Difícil; os números e o que mudou estão na produção da §5.9.
