...
- **As naus** são postas pelo `setup` (`place`, exato em [18,112]) para ganhar nome (G8: "Naus de Micenas") e **piso de vida de 30 %** (G9): um ataque ao cais não afunda a missão. O embarque usa **`remove`** (G6) e **raio 5**, não 4: 30 cidadãos mandados ao Mercado ficam 28 a ≤ 4 tiles do centro dele e 30 a ≤ 5 (os empurrados para fora esperariam para sempre). O segredo `chama` usa o mesmo raio e é oculto com `done` direto (G1).
- **Dificuldades (G3), como a ficha sugeria:** meta **20 / 30 / 40**; naus de **7-7-6** no Fácil (senão a meta de 20 se cumpria aos 9 min, abaixo da janela), **10-10-10** no Normal e **10-10-10-10** no Difícil, com a 4ª aos **14 min** (aos 15, o Culto "muito difícil" chegava ao cais antes do último embarque e o arconte caía aos 15m21s); **Cronos aos 14 / 12 / 10 min** e Prometeu 1 min antes; as ondas e a vanguarda escalam (`raid` e `spawn { scaled }`). O painel mostra as contagens das naus e de Cronos da dificuldade e as barras "A bordo" e "Lugares nas naus atracadas" (G4). Por isso o objetivo `exodo` não traz número ("Embarque o povo nas naus de Náuplia antes que zarpem") e a 2ª fala da intro diz "cada vida salva", não "trinta".
- **As ondas vêm do jogador 2** (a marionete Cronos), não do Culto (IA): a IA absorveria as unidades do `raid` no exército dela. A 3ª onda sai do nordeste (ângulo 7, pela Via) porque o sudeste do cais aqui é o golfo. Quem sobra de uma onda, parado, **vai caçar o arconte** (gatilho `hostes`) — é a cobrança de "proteger o líder". Os poderes vêm por `set minorGods` (Afrodite, Hera, Hermes; Atena com Cronos), como na ficha, e o **2º Raio** (G11 `powerUsed` + `reset`) volta uma vez depois dos 5,5 min.
- **Acréscimos:** a **vanguarda de Lícaon** (7,5 min, 12 soldados com 2 catapultas, escalada, descendo a Descida rumo à porta norte; o vilão fala) e as pesquisas militares do Culto, para que algo além das três ondas bata na cidade antes de Cronos (a IA que joga no teste ainda a segura com folga; para quem joga, é mais uma frente enquanto organiza as levas); falas curtas da Pítia (a dica com o botão direito e o arconte de coroa, o primeiro a bordo, o arconte ferido, a queda do CC, os 2 minutos finais), do Batedor (as quatro naus, a vanguarda), de Zeus (o templo que arde, o 2º Raio) e de Atena (a Restauração).
...
- **Vitória** = os objetivos `exodo` e `arconte` cumpridos (1 s depois da contagem; assim a tela de fim os mostra cumpridos). **Travas (G6):** ninguém ergue o Portal dos Titãs (nada de 2º Prometeu nem 2º Cronos), Idade máxima Mítica, Jasão e Aquiles fora (m8, m7), o Culto sem heróis gregos.
...
| m11 | Ninguém embarca: **derrota** aos 18 min (ou antes, se o arconte cair). **Medido (produção, `npx tsx scripts/missions.ts 14 m11_chamas`):** o arconte, parado junto ao CC, cai no 1º ataque do Culto: derrota aos **6m51s / 9m14s / 8m06s** (Fácil / Normal / Difícil; o Culto "fácil" ataca antes, com menos soldados); as ondas da passiva (2, até a derrota) têm unidades | Arconte e cidadãos fazem `move` até `naus` em 3 levas (aos 200, 500 e 740 s); poderes usados nos grupos das ondas. **Produção:** levas uma por nau, 70 s antes dela, com os lugares + 2 (a última leva leva todos e sai 80 s antes de Cronos: 170 / 470 / 640 s no Normal, 710 s no Fácil, 520 s no Difícil); o arconte e o exército inicial descem com a 1ª leva, o arconte fica atrás do cais em postura passiva e a escolta guarda a boca do porto; na última leva, o exército da cidade também desce; cada poder na onda dele (detalhes abaixo) | 18 min (§4, fixo) · janela estrita **12m36s–23m24s** (a vitória só pode sair antes dos 18). **Medido:** vitória aos **13m11s / 13m19s / 14m19s** (Fácil / Normal / Difícil) — o relógio das naus (3ª aos 13 min; 4ª aos 14 no Difícil) mais o embarque; os quatro objetivos cumpridos nas três (`retaguarda` e o segredo `chama` também); Maldição na 1ª onda (121 s) e Tempestade nos arqueiros que caçam o arconte (331 s) nas três, Trégua na cavalaria do cais no Fácil e no Normal, 2º Raio no Difícil; 3 invasões, todas com unidades |
