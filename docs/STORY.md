...
| **G4** ✅ | HUD e tempo relativo | **Feito.** `Value { "time": true }` (segundos inteiros de jogo) marca instantes com `setVar`; `hud.countdown { "fromVar" }` conta `seconds` a partir da marca (sem marca, não aparece); `hud.progress { "var", "max": Valor, "while"?, "format"?: "percent" \| "count" \| "time" }` desenha a barra de uma variável (a de obra, `{ "entity" }`, ganhou `while`/`format`). HUD em `src/ui/scenario-hud.ts` (função pura, rótulos PT/EN); o lint avisa `var`/`fromVar` que ninguém escreve | Tempo desde a marca: `{ "time": { "gte": { "add": [ { "var": "t0" }, 600 ] } } }` | — | m6 (guarda ✅), m10 (altar), m11 (embarcados) | **feita** |
