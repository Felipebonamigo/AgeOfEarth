...
- **Objetivo secreto:** fica `hidden: true`, **sem** `done`/`failed`. Um **gatilho espelho** o cumpre com `{ "do": "objective", "id": …, "status": "done" }`. Motivo: o runner **não avalia objetivos ocultos** (`runner.ts` pula `sc.hidden[id]`). Quando [G1] existir, basta mover a condição para `done`.
...
- **Tag de grupo:** só `spawn` com `tag` grava o grupo inteiro (`#tag[k]`). Entidades do mapa fixo com a **mesma** tag ficam só com a última (`game.ts` usa `tags.set`) [G5]. Por isso, nos mapas fixos, **uma tag por entidade** (`corrente1`, `corrente2`…), e grupos nascem por `spawn` no `setup`.
...
- **Derrota por CC:** `{ "all": [ { "time": { "gte": 5 } }, { "buildings": { "player": 0, "type": "town_center" }, "eq": 0 } ] }` (chamada abaixo de `DERROTA_CC`). É obrigatória: dentro de cenário, `alive` nunca muda e a derrota implícita não dispara [G2].
...
|---|---|---|---|---|---|---|
| **G0** | **Registro de missões JSON na aba Campanha** (infraestrutura, não gramática) | `SCENARIOS` só aceita TS; `RESERVED_SCENARIO_IDS` só tem m1–m3; menu com lista plana; conquistas só do prólogo | Lista `CAMPAIGN = [{ act, id, file }]` que compila JSON (`compileScenario`); cabeçalhos por ato em `menu.ts`; ids reservados; conquistas "Ato I/II/III" e uma por missão; selo "Prólogo" em m1–m3 | 6–8 h | **todas** (sem isso nenhuma missão nova vai ao ar) | **obrigatória, semana 0** |
| **G1** | Objetivo oculto nunca é avaliado | `runner.ts` pula `sc.hidden[id]` | Avaliar também os ocultos. Ao mudar de estado, `ctx.objective` já os revela | 1 h | simplifica os segredos de todas (hoje: gatilho espelho) | alta |
| **G2** | Fim de partida em cenário | `game.ts`: `runScenario` **ou** `checkVictory`; `alive` nunca muda; Maravilha, Rei da Colina e Regicídio não encerram | Rodar a parte de **eliminação** de `checkVictory` em cenário, sem declarar vencedor. Condições `{ "koth": { "team": 0 }, "gte": 120 }` (`state.koth` já é atualizado), `{ "wonderHeld": { "player": 0 }, "gte": 360 }` e `{ "kingAlive": 0 }` | 3–4 h | m6, m10, m11 (tira os contadores); protege qualquer missão sem `defeat` | alta |
| **G3** | Condição de dificuldade | Só `raid` escala (`scaledGroup`), e as IAs sobem um degrau | `{ "difficulty": "easy" \| "normal" \| "hard" \| [...] }`, `Value { "stat": "difficulty" }` e `spawn { "scaled": true }` | 2–3 h | as variações "com [G3]" de todas | alta |
...
| **G5** | Tag de grupo em entidades do mapa | `game.ts` faz `tags.set` e a última sobrescreve | Acumular `#tag[k]` também para `map.entities` | 1 h | libera grupos desenhados no editor (hoje: `spawn` no `setup`) | média |
...
| **G7** | Lint de cenário (ferramenta) | nada impede `exists:false` sobre tag futura | Aviso de `validateScenario`: tag usada em condição e criada só por gatilho, sem `fired` no mesmo `all` | 2 h | todas (evita objetivos cumpridos no segundo 1) | média |
...
**Correções técnicas no prólogo** (bugs, não roteiro; decisão do dono, fora do escopo desta etapa):
- a) `m3_portal`: os "Aliados de Poseidon" não têm `team` e hoje são **hostis** a todos (`team: pc.team ?? i`). Correção: `team: 0` para Argos e aliados.
- b) `m1_despertar`: não tem `defeat`. Hoje, quem perde tudo fica numa partida sem fim. [G2] resolve isso sem mexer na m1, e um `defeat` explícito também resolveria.
- O texto e o roteiro das três missões continuam idênticos nos dois casos.
