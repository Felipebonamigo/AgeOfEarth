...
- **A pedra do Ótris só cede às máquinas de Argos** ([G9] `hpFloor`). Muros, portões e Pilares nascem com piso de vida 1 (não sofrem dano) e o perdem enquanto houver uma helépole ou um petróbolo de Argos a até 12 tiles (`bronze_mortal`/`bronze_divino`, `anel_mortal`/`anel_divino`, `pilarN_mortal`/`pilarN_divino`, com as variáveis `ext_mortal`, `int_mortal` e `pN_mortal` para só agir nas trocas). Sem isso, as ondas da IA de Hades (normal, igual nas três dificuldades) derrubavam o Portão de Bronze aos 5 min e, sozinhas, venciam a missão na passiva aos 22m43s. Com as máquinas por perto, todo golpe conta, inclusive os de Hades. A 1ª versão exigia só "um militar de Argos" por perto; com ela, qualquer soldado que perseguisse um inimigo até a rampa abria o portão para as legiões de Hades. Cada muro tem o seu piso (`pick: "nearest"` num ponto do centro do tile, [70,5; 52,5] etc.: num ponto de canto, dois muros vizinhos empatavam e dois ficavam sem piso — foi por um deles que o Culto saiu numa das medições). Hades avisa uma vez (`hades_bronze`: "traga as suas máquinas").
- **As colmeias dos Pilares** ([G9] `hp`): "três Pilares que zumbem como colmeias". Com 3/4, 1/2 e 1/4 da vida, cada Pilar solta um enxame de 8 Sentinelas (mirmidões, arqueiros cretenses, um Ciclope e uma Medusa; `colmeiaNa/b/c`, tags `enxameNa/b/c`), e a pedra não cede enquanto o enxame viver. Sem elas, com a pedra cedendo, 50–60 militares derrubavam cada Pilar em 15–25 s.
- **Guarnição das Sentinelas.** No santuário, duas guardas de 16 (mirmidões, hipaspistas, arqueiros cretenses, Medusas e um Cérbero) e um reforço de 6 a cada 90 s a partir dos 4 min, **no máximo 12 vezes** (`repeat: { max: 12 }`, [G17]): sem teto, o reforço empatava com o cerco e o roteiro ficava em jogo até os 55 min. No altar, uma guarda de 7 (hipaspistas, arqueiros e um Ciclope, `scaled` [G3]): trancado, o Culto não disputa a colina, e a guarda dá ao altar alguém para expulsar.
- **O altar exige Argos nele** ([G2] `koth`): a conta sobe quando a colina é do time de Argos (sem inimigos a 6 tiles, a regra do modo) **e** há um militar de Argos nela. Com o texto da ficha (Argos ou Hades), as legiões de Hades tomavam o altar aos 3 min sozinhas, a passiva cortava as sortidas e nunca perdia. A barra do painel mostra a conta ([G4], formato tempo). No topo da tela, em cenário, a colina mostra só quem a segura e há quanto tempo ("⛰ Colina: Argos & Hades 94s"), sem a promessa de vitória em 240 s do modo, que não roda em cenário (`src/ui/hud.ts`, `src/core/sim/modes.ts`, chaves `top.kothScenario` e `ev.kothTakenScenario`).
- **Sortidas por dificuldade** ([G3]): a mesma composição da ficha (3 hipaspistas, Ciclope, Medusa, Cérbero, em `raid` contra o CC de Argos, ângulo 7), com os hipaspistas primeiro — o `raid` do Fácil fica com os 4 primeiros e o do Difícil soma a primeira metade, então o Difícil ganha 3 hipaspistas, não mais 3 míticas. Fácil a cada 3 min desde os 6 (`sortida_facil`), Normal a cada 4 min 30 s desde os 9 (`sortida`), Difícil a cada 5 min 30 s desde os 11 (`sortida_dificil`): a sortida do Difícil (9) derruba sozinha um acampamento parado, e com a cadência da ficha (a cada 2 min 30 s desde os 5) a passiva perdia aos 11 / 10 / 8 min. A fala do Batedor vale para qualquer uma. A variação de altar da ficha (90/120/180 s) ficou de fora: o objetivo diz "2 minutos" e a conta é a mesma nas três. O "kill no pilar3 no Fácil" também ficou de fora: os tempos já saem na ordem das dificuldades.
- **Hades trouxe as legiões dele** (intro): 4 hipaspistas, 4 arqueiros cretenses e 2 Cérberos no `setup` (`legiao`). Só com o kit, a IA do Culto (antes de trancado) arrasava Hades aos 4 min 30 s.
...
- **Falas acrescentadas** (tom da §1.2, ≤ 200 caracteres): a Pítia apresentando o altar e as teclas do cerco (W e Q, mecânica nova) e explicando as colmeias; o Batedor na 1ª sortida; Hades pedindo o altar, diante do bronze sem máquinas e se o acampamento dele cair; Hades quando o anel interno cede; Cronos no 1º e no 2º Pilar ("Três mil anos de espera... e agora só uma pedra"), com a Pítia ("Ele não teme as pedras que caem, arconte. Ele as conta."). O `outro` da ficha foi dividido em dois parágrafos, sem mudar o texto; os objetivos dizem onde fica cada coisa.
...
| m10 | Sortidas até o CC cair: **derrota** em 15–25 min. **Medido (produção, `npx tsx scripts/missions.ts 26 m10_otris`):** derrota aos 21m17s / 18m57s / 17m18s (Fácil / Normal / Difícil), com 6 / 3 / 2 sortidas (todas com unidades); o Culto, trancado, não sai; com a passiva padrão de 14 min, em jogo nas três | Exército no altar até `altar_s` = 120; helépoles em `portao_bronze` e depois nos pilares. **Produção:** a IA do jogador segura o altar sozinha (o Rei da Colina reúne o exército lá; `altar` cumprido aos ~2m30s); os passos põem as máquinas de cerco (4 helépoles e 2 petróbolos), o Mercado quando falta ouro e o cerco por estado (detalhes abaixo) | 25–40 min · janela estrita 21m–45m30s · **medido: 27m17s / 37m54s / 40m27s** |
...
- Roteiro da **m10** (`MISSION_SCRIPTS.m10_otris`, janela 21m–45m30s, `hold` sempre): no Rei da Colina a IA do jogador reúne o exército no Altar do Tempo e o segura sozinha (expulsa a guarda das Sentinelas e cumpre o `altar` aos ~2m30s, antes da 1ª sortida). Os passos: cidadãos até 45, Casas antes do teto de população (`m9House`), filas com `M10_MIX` (hipaspistas, mirmidões, arqueiros cretenses e as míticas dos três deuses menores: Minotauro, Mantícora e Medusa) só até 70 militares de linha (`M10_LINE_CAP`), e as **máquinas à parte** (`m10Engines`: 4 helépoles e 2 petróbolos na Oficina, antes do exército de linha); o **Mercado** quando o ouro cai abaixo de 300 (vende o maior estoque acima de 700; sem ele, o Difícil ficava com 25 mil de comida e de madeira e 30 de ouro, sem treinar); poderes (`battlePowers`) e fuga da Tempestade. O **cerco** (`m10AssaultTarget`/`m10Assault`, a cada 5 s): um alvo por vez — o Portão de Bronze, o portão interno e o Pilar mais perto do exército —, começando com ≥ 60 militares dos quais ≥ 3 máquinas (a pedra só cede perto delas), ou com o exército cheio (≥ 75, perto do limite de população: sem isso ele parava em 84 militares sem nenhuma helépole e ficava em jogo até os 55 min no Fácil e no Difícil), e continuando enquanto ≥ 12 militares estiverem a até 18 tiles do alvo; as máquinas batem direto no alvo e o resto vai em ataque-movimento, e perto dele (10 tiles), sem militares inimigos a 8, todos batem nele. Fora do cerco, quem ficou na encosta volta ao altar (`m10Regroup`). Medido: o 1º cerco (~12 min) abre o Portão de Bronze (11m45s–13m56s) e em geral o anel interno; os Pilares, as colmeias e o reforço do santuário custam mais uma ou duas levas — 27m17s / 37m54s / 40m27s. O roteiro é caótico como os outros (§7.2, "Estrito por padrão"): as medições de calibração com a mesma missão e pequenas mudanças de números iam de 15 a 55 min, e o que deu a forma final foi tirar o que era bimodal (o 1º cerco vencer ou não) — as colmeias com piso de vida, o reforço com teto e o Culto trancado.
