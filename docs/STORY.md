...
- **Atrito nos platôs 2 e 3 por Santuários**: o território do Culto (CC em [82,24], Fortaleza em [74,30]) não alcança os platôs; um Templo do Culto ali atrairia os sacerdotes da IA. Cada platô ganhou um Santuário (Templo do jogador 2, Hades: território 5, atrito 0,7/s); as torres-corrente projetam 3.
- **2ª cidade no Difícil**: dois objetivos ocultos com o mesmo texto, revelados no 1º segundo por G3 — `cidades` (opcional, Fácil/Normal) e `cidades_dificil` (principal, Difícil; a vitória o exige). A Águia renasce 180 s depois de cada morte no Difícil (contador `aguia_s`).
...
- **Falas acrescentadas** (tom da §1.2, ≤ 200 caracteres): Zeus no Difícil (a 2ª cidade), Pítia no Fácil (a 3ª corrente cedeu), Pítia ao fundar a colônia (o exército já ocupa 32 de população: Casas antes de cidadãos), Héracles ao avistar a Águia, Pítia quando a Águia renasce; a profecia de Prometeu vem quando ele chega à Fortaleza (ou quando ela cai). A vingança da `corrente3` é 3 hipaspistas, 2 toxotas e 1 Cérbero (não no Fácil). HUD: progresso da obra do CC da colônia. Segredo `hefesto` com `done` direto ([G1]).
...
- Roteiro da **m4** (`MISSION_SCRIPTS.m4_caucaso`, `hold` sempre): 2 cidadãos erguem o CC em [48,84] no 1º segundo (antes que a IA do jogador funde a cidade na praia; com 2 construtores a colônia só se cumpre depois do 1º minuto), o resto sobe ao vale; cidadãos até 42 e filas militares cheias (`M4_MIX`); Héracles guarnecido no CC (se ele cair, a missão está perdida); 3 torres na saída do desfiladeiro; 3 mineiros vão à lenha quando falta madeira e sobra ouro; no Difícil, Civismo I e a 2ª cidade no vale lateral; fora dos assaltos, quem sobe o desfiladeiro volta ao vale; assaltos às correntes a partir de 15, 18 e 21 min (≥ 26/28/28 militares) e, livre Prometeu, à Fortaleza. Sem as torres e a lenha, a IA "difícil" perdia no Difícil (onda do Culto aos ~9 min).
