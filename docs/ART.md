...
**Arquivos**: `src/render/art/types.ts` (contrato do manifesto/atlas), `logic.ts` (puras: direção 8 com histerese, animação por estado, quadro a 10 fps, nomes, estágio de obra, espécie/variante/estágio de props, checagem de `meta.aoe`, espelhamento, cor composta), `AtlasSource.ts` (manifesto + atlas por grupo e escala com `Assets.load`, prefixo de cache por arquivo, mipmaps, recusa de `pxPerTile`/`pitchDeg`/versão/passe fora do contrato; sem fetch para `file://` cai em XHR + `<img>` + `Spritesheet`), `ProceduralSource.ts` (o `TextureCache` de sempre), `ArtLibrary.ts` (`unit(id)`, `frames/teamFrames/shadowFrames(art, anim, dir)`, `building(id, estágio)`, `prop(nome)`, `prewarm()`, `ready()`, `generation`), `src/render/views/UnitView.ts` e `BuildingView.ts`, `src/render/props.ts` (a camada de nós saiu de `renderer.ts`), `tests/art-library.test.ts`, `scripts/artparade.mjs` (desfile + conferência de animações/direções), `scripts/rendercpu.mjs` (CPU do renderizador).

**Decisões da parte B**:
- *Carregamento*: o manifesto é lido no `setQuality` (≈ 5 KB); os atlas de units/buildings/props da escala servida começam a carregar no `setState` (início da partida ou editor), em paralelo, sem esperar — enquanto isso tudo sai procedural. Um tipo que aparece antes do pré-aquecimento dispara o grupo dele. A `generation` só muda quando **nada** mais está carregando, então o renderizador refaz vistas e props uma vez por leva (não uma por atlas). Trocar 1× ↔ 2× (preset alto) serve a escala antiga até a nova ficar pronta e só então a descarrega (`collect()` depois da reconstrução: texturas do atlas são compartilhadas e nunca destruídas por vista).
- *Ordem global por y*: com a arte assada, unidades (não voadoras) e edifícios entram nas faixas de chunk dos props, com `zIndex` = y do pé (props = centro do tile, edifícios = centro da área, tocos meio tile acima para ficarem sob quem pisa neles); voadoras continuam na camada `units`, acima. A camada `ground` (seleção, waypoints, relíquias, colina) passa para baixo das faixas; o fantasma de construção e o alvo de poder vão para a camada `hp` (senão árvores e o templo os cobririam). Com a opção desligada a ordem das camadas é exatamente a de antes.
- *Direção*: velocidade convertida para a tela pela câmera (`Camera.worldDeltaToScreen`, hoje identidade em escala), `dir = ((round(ângulo/(π/4)) % 8) + 8) % 8` com histerese de 0,12 rad além da borda do octante (sem piscar em diagonais); parada mantém a última; atacando/coletando/construindo vira para o alvo.
- *Animação*: ataque em curso > andar (ou `carry` com carga ≥ 1) > `gather` (coletando/construindo/reparando parado) > parado; cada `attackTick` novo recomeça o ataque no quadro 0; loops usam o relógio com uma fase por unidade (não andam em sincronia). Morte: a vista que some deixa a direção para o efeito `death`, que toca `die` na faixa do pé (alfa 1 na primeira metade do efeito, some na segunda); petrificada = estátua cinza (quadro parado) no lugar da queda; colapso do templo usa o quadro completo em cinza.
...
- *Barras de vida e patente*: acima do topo visível dos quadros de parado (hoplita ≈ 43 px com a lança, cidadão ≈ 26 px), não em `anchor.y · sourceSize.h` (a moldura inclui lança deitada, morte e sombra e deixaria a barra 20–30 px acima do elmo). Templo: acima do telhado do estágio atual.
- *Pick*: 1º a caixa do quadro de cor atual das unidades assadas (a de pé mais à frente ganha), 2º o círculo de sempre, 3º o tile do edifício, 4º a caixa do quadro do edifício assado (telhado). Unidades vêm antes de qualquer edifício, então quem está atrás do templo continua clicável; a névoa decide pelo pé.
...
- *Desempenho* (números em `docs/QA.md`): o mundo é um grupo de render do Pixi 8 (mover a câmera não recalcula a transformação de cada sprite) e props, sombras dos props e cada faixa (modo assado) são grupos próprios — reordenar unidades refaz só as instruções da faixa delas. Árvores no miolo do bosque (E, S e SE também árvores) não desenham a sombra (cairia sob as copas vizinhas; volta quando a vizinha é cortada): −20 % de sprites num bosque denso. Com isso nosso código custa o mesmo que antes e o Pixi fica perto da base a zoom 1 e na rolagem (+0–0,1 ms), +0,2 ms no aglomerado e +0,8 ms (+50 %) no mapa inteiro com ~4 900 sprites; o fps por software no preset baixo empata ou melhora. Draw calls sobem de 3–6 para 8–15 (cada grupo é um lote), dentro do orçamento de 40.
