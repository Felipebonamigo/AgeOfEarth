...
**Pendências da Etapa 3**: os 8 edifícios restantes do lote militar/maravilhas (quartel, estábulo, oficina de cerco, fortaleza, 3 maravilhas, portal dos titãs com abertura animada); captura de cidade por Idade (4) para o dono aprovar; retratos (`renderer.portrait`) ainda procedurais; a linha de muralha arrastada usa Bresenham 8-conexo — degraus diagonais não se ligam no bitmask N/L/S/O (saem como pilares vizinhos; decidir se a linha vira 4-conexa, pergunta 17); sombras que se sobrepõem entre peças diferentes (torre × muralha, cantos internos) ainda somam no multiply (uma camada de sombras com união por alfa resolveria, a medir no orçamento); medir `renderperf`/`rendercpu` com a cidade (muitas muralhas = muitos sprites pequenos).
