...
| 3 ⏳ | **Edifícios (21)** (em andamento: **base concluída** — bake genérico de edifícios com obra 0–2, pronto, dano 1–2 genérico, portão aberto, escombros por pegada, variantes por bitmask/eixo/Idade, sombra recortada e vizinhos de contexto, ícones 64² no atlas `icons`; `BuildingView` genérico, fumaça por partículas, escombros, fantasma assado, ícones no HUD; lote 1 = Centro Cívico ×3 Idades, casa, muralha ×16, portão ew/ns, torre, + dano/ícone do templo; `scripts/artcity.mjs`, `scripts/bake/compose.mjs`, capturas `docs/art/etapa3-*`; ver Apêndice D) | 50 | Rigs de edifícios: obra 3 estágios, dano 2 + fumaça, escombros, muralha por bitmask (16), portão, portal dos titãs animado, fazenda 3 estágios, estandartes, fantasma de construção, desabilitado; ícones assados de cada um no HUD (com fallback) | `ProceduralSource` deixa de ser usado para edifícios em partida normal; muralhas contínuas sem costura; obra visível 0/33/66 %; captura de uma cidade completa por Idade (4 capturas) aprovada pelo dono |
...
## Apêndice D — Etapa 3 em andamento: a base dos edifícios e como adicionar um edifício
...
4. `npm run art:check` e `npx vitest run tests/art-manifest.test.ts tests/art-library.test.ts` (a lista do lote em `art-library.test.ts` "edifícios assados (lote 1)" pode ganhar o id); `npm run build`, `npm run preview` e `node scripts/artcity.mjs` (ou uma cena própria) com a captura olhada com Read. Nenhuma linha do renderizador muda: o tipo passa a sair assado quando o atlas tiver os 6 estados.
...
**Pendências da Etapa 3**: os edifícios restantes (lote econômico: fazenda com 3 estágios de plantio e plana/pisável, celeiro, serraria, mina, mercado, academia, cornucópia); do lote militar: as estátuas (Zeus, Colosso) são figuras de primitivas — boas a zoom 1, bonecas a 2,2 (candidatas a `.glb`) —, os ícones do Colosso e do portal enquadram o modelo inteiro (a estátua fica pequena em 64 px) e a VRAM do 2× cresceu (sombras em 3 páginas; KTX2 ou sombras a 1× se o orçamento apertar); captura de cidade por Idade (4) para o dono aprovar; retratos (`renderer.portrait`) ainda procedurais; a linha de muralha arrastada usa Bresenham 8-conexo — degraus diagonais não se ligam no bitmask N/L/S/O (saem como pilares vizinhos; decidir se a linha vira 4-conexa, pergunta 17); sombras que se sobrepõem entre peças diferentes (torre × muralha, cantos internos) ainda somam no multiply (uma camada de sombras com união por alfa resolveria, a medir no orçamento); medir `renderperf`/`rendercpu` com a cidade (muitas muralhas = muitos sprites pequenos).
