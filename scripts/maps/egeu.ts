// Mapa oficial "Egeu" (2v2, 112×112), desenhado com as operações do editor (scripts/maps/lib.ts).
...
import { LOCAL_RADIUS, MapBuilder, T, d2, dist, mirrorXY, placeStartLayout, writeMap, type Pt } from './lib';

const W = 112, H = 112, SEED = 8989;
const CX = (W - 1) / 2, CY = (H - 1) / 2;            // 55,5: eixos de simetria
...
export function buildEgeu(): { file: FixedMapData; warnings: MapIssue[] } {
  ...
  const shoals = [{ x: 27, half: 2.6 }, { x: CX, half: 3.1 }, { x: W - 1 - 27, half: 2.6 }];
  ...
  b.terrain((x, y) => {
    ...
    const shoal = shoals.find((s) => Math.abs(x - s.x - (s.x === CX ? 0 : (n.noise(3.1, y * 0.18) - 0.5) * 4)) <= s.half + (n.noise(x * 0.5 + 5, y * 0.5) - 0.5) * 0.9);
    if (fromAxis <= sea) {
  ...
  b.nodes('gold', [[26, 55], [27, 55]]);                                        // ilha oeste: 2×2 no eixo (4 veios; leste pelo espelho)
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) {
  const { file, warnings } = buildEgeu();
  writeMap(file, warnings, process.argv[2] ?? 'src/core/data/maps/egeu.map.json');
}
