...
import { PX_PER_TILE, SSAA, M2T, dirYaw, makeCamera, makeLights } from './camera.js';
 ...
 * Devolve `{ color, team, shadow }` em RGBA (Uint8Array) ou null para passes desligados/vazios.
 */
function renderPasses(model, box, { scale, team, shadow }) {
  ...
  const cam = makeCamera(THREE, { wTiles: box.w / ppt, hTiles: box.h / ppt, anchor: [box.ax / box.w, box.ay / box.h] });
  model.updateMatrixWorld(true);
  forEachMesh(model, (o) => { o.layers.set(LAYER_MODEL); o.castShadow = true; o.receiveShadow = true; });
...
async function sourceFor(manifest, poses, state, f) {
  ...
  if (src.rig === 'building') {
    ...
    if (!state.buildings.has(key)) state.buildings.set(key, buildBuilding(THREE, M, src.params?.style ?? manifest.id, { ...(src.params ?? {}), ...(f.params ?? {}), frame: f.frame, frames: f.frames }));
...
export async function bakeBatch(job) {
  ...
  for (const f of job.frames) {
    ...
    if (ext !== extent) { setLights(ext); extent = ext; }
    const { model, pose } = await sourceFor(manifest, poses, state, f);
    scene.add(model);
    pose(f);
    const r = renderPasses(model, f.box, { scale, team: manifest.team, shadow: manifest.shadow });
    scene.remove(model);
