...
export const BUDGET = {
  ...
  maxVramMB: 250,           // texturas residentes, pior caso (§6), POR ESCALA: uma partida usa só uma (1× ou 2×, pelo preset)
...
export function runCheck(root: string): CheckResult {
  ...
  // o teto vale por escala (Etapa 4, lote heróis): as duas escalas juntas nunca estão na GPU ao mesmo tempo, e com o
  // carregamento por tipo uma partida só sobe os tipos em uso — este é o pior caso de uma escala com TUDO carregado
  for (const [s, v] of Object.entries(stats.vramByScale)) if (v > BUDGET.maxVramMB * 1048576) errors.push(`atlas ${s}× somam ${(v / 1048576).toFixed(1)} MB de VRAM > ${BUDGET.maxVramMB} MB`);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const s = r.stats;
  const byScale = Object.entries(s.vramByScale).map(([k, v]) => `${(v / 1048576).toFixed(1)} a ${k}×`).join(', ');
  console.log(`art:check — ${s.manifests} manifestos` + (s.hasArtifacts ? `, ${s.atlases} atlas, ${s.frames} quadros, ${(s.pngBytes / 1048576).toFixed(2)} MB de PNG, ${(s.vramBytes / 1048576).toFixed(1)} MB de VRAM (${byScale}; orçamento ${BUDGET.maxPngMB} MB / ${BUDGET.maxVramMB} MB por escala)` : ' (public/art ainda não gerado)') + (r.errors.length ? ` — ${r.errors.length} erro(s)` : ' — ok'));
