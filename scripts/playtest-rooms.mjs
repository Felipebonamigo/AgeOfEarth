...
const mk = async (name) => {
  ...
  await page.fill('#mp-url', 'ws://localhost:8787'); await page.fill('#mp-name', name);
  return page;
};
...
// sala cheia/iniciada some: o anfitrião inicia
await host.click('#mp-start'); await host.waitForTimeout(2500);
await third.click('#mp-browse'); await third.waitForTimeout(1000);
console.log('sala iniciada não aparece:', (await third.textContent('#mp-rooms'))?.includes(room) ? 'FALHOU' : 'ok', '| partida rodando no convidado:', await guest.evaluate(() => !!window.aoe.session) ? 'ok' : 'FALHOU');
