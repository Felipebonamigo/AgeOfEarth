...
import { LocalScheduler } from '../core/net/lockstep';
...
export class Session {
  ...
  static newGame(config: GameConfig): Session { return new Session(createGame(config), config.players.findIndex((p) => !p.isAI)); }
  ...
  step(dtReal: number): number {
    ...
    let n = 0;
    while (this.accumulator >= DT && n < 12) { this.scheduler.step(this.state); this.accumulator -= DT; n++; }
